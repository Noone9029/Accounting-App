export const PERMISSIONS = {
  dashboard: {
    view: "dashboard.view",
  },
  organization: {
    view: "organization.view",
    update: "organization.update",
  },
  users: {
    view: "users.view",
    invite: "users.invite",
    manage: "users.manage",
  },
  roles: {
    view: "roles.view",
    manage: "roles.manage",
  },
  accounts: {
    view: "accounts.view",
    manage: "accounts.manage",
  },
  currencies: {
    read: "currencies.read",
    manage: "currencies.manage",
  },
  fxRates: {
    read: "fxRates.read",
    manage: "fxRates.manage",
  },
  fxRevaluation: {
    read: "fxRevaluation.read",
    run: "fxRevaluation.run",
    reverse: "fxRevaluation.reverse",
  },
  bankAccounts: {
    view: "bankAccounts.view",
    manage: "bankAccounts.manage",
    transactionsView: "bankAccounts.transactions.view",
    openingBalancePost: "bankAccounts.openingBalance.post",
  },
  bankTransfers: {
    view: "bankTransfers.view",
    create: "bankTransfers.create",
    void: "bankTransfers.void",
  },
  bankStatements: {
    view: "bankStatements.view",
    import: "bankStatements.import",
    previewImport: "bankStatements.previewImport",
    reconcile: "bankStatements.reconcile",
    manage: "bankStatements.manage",
  },
  bankReconciliations: {
    view: "bankReconciliations.view",
    create: "bankReconciliations.create",
    approve: "bankReconciliations.approve",
    reopen: "bankReconciliations.reopen",
    close: "bankReconciliations.close",
    void: "bankReconciliations.void",
  },
  bankIntegrations: {
    connectionManage: "bankIntegrations.connection.manage",
    feedRead: "bankIntegrations.feed.read",
    beneficiaryManage: "bankIntegrations.beneficiary.manage",
    vendorPaymentCreate: "bankIntegrations.vendorPayment.create",
    vendorPaymentApprove: "bankIntegrations.vendorPayment.approve",
    vendorPaymentReconcile: "bankIntegrations.vendorPayment.reconcile",
  },
  taxRates: {
    view: "taxRates.view",
    manage: "taxRates.manage",
  },
  journals: {
    view: "journals.view",
    create: "journals.create",
    post: "journals.post",
    reverse: "journals.reverse",
  },
  fixedAssets: {
    read: "fixedAssets.read",
    manage: "fixedAssets.manage",
    categoriesManage: "fixedAssets.categories.manage",
    capitalize: "fixedAssets.capitalize",
    depreciationReview: "fixedAssets.depreciation.review",
    depreciationPost: "fixedAssets.depreciation.post",
    dispose: "fixedAssets.dispose",
    reports: "fixedAssets.reports",
  },
  fiscalPeriods: {
    view: "fiscalPeriods.view",
    manage: "fiscalPeriods.manage",
    lock: "fiscalPeriods.lock",
  },
  accountingClose: {
    read: "accountingClose.read",
    manage: "accountingClose.manage",
    prepare: "accountingClose.prepare",
    review: "accountingClose.review",
    close: "accountingClose.close",
    lock: "accountingClose.lock",
  },
  contacts: {
    view: "contacts.view",
    manage: "contacts.manage",
    sendCustomerStatements: "contacts.sendCustomerStatements",
    sendSupplierStatements: "contacts.sendSupplierStatements",
  },
  items: {
    view: "items.view",
    manage: "items.manage",
  },
  inventory: {
    view: "inventory.view",
    manage: "inventory.manage",
    cogsPost: "inventory.cogs.post",
    cogsReverse: "inventory.cogs.reverse",
    receiptsPostAsset: "inventory.receipts.postAsset",
    receiptsReverseAsset: "inventory.receipts.reverseAsset",
    varianceProposalsView: "inventory.varianceProposals.view",
    varianceProposalsCreate: "inventory.varianceProposals.create",
    varianceProposalsApprove: "inventory.varianceProposals.approve",
    varianceProposalsPost: "inventory.varianceProposals.post",
    varianceProposalsReverse: "inventory.varianceProposals.reverse",
    varianceProposalsVoid: "inventory.varianceProposals.void",
  },
  warehouses: {
    view: "warehouses.view",
    manage: "warehouses.manage",
  },
  stockMovements: {
    view: "stockMovements.view",
    create: "stockMovements.create",
  },
  stockAdjustments: {
    create: "stockAdjustments.create",
  },
  inventoryAdjustments: {
    view: "inventoryAdjustments.view",
    create: "inventoryAdjustments.create",
    approve: "inventoryAdjustments.approve",
    void: "inventoryAdjustments.void",
  },
  warehouseTransfers: {
    view: "warehouseTransfers.view",
    create: "warehouseTransfers.create",
    void: "warehouseTransfers.void",
  },
  purchaseReceiving: {
    view: "purchaseReceiving.view",
    create: "purchaseReceiving.create",
  },
  salesStockIssue: {
    view: "salesStockIssue.view",
    create: "salesStockIssue.create",
  },
  salesInvoices: {
    view: "salesInvoices.view",
    create: "salesInvoices.create",
    update: "salesInvoices.update",
    finalize: "salesInvoices.finalize",
    void: "salesInvoices.void",
    send: "salesInvoices.send",
  },
  customerPayments: {
    view: "customerPayments.view",
    create: "customerPayments.create",
    applyUnapplied: "customerPayments.applyUnapplied",
    reverseUnappliedAllocation: "customerPayments.reverseUnappliedAllocation",
    receiptPdfGenerate: "customerPayments.receiptPdf.generate",
    send: "customerPayments.send",
    void: "customerPayments.void",
  },
  creditNotes: {
    view: "creditNotes.view",
    create: "creditNotes.create",
    finalize: "creditNotes.finalize",
    send: "creditNotes.send",
    void: "creditNotes.void",
  },
  customerRefunds: {
    view: "customerRefunds.view",
    create: "customerRefunds.create",
    void: "customerRefunds.void",
  },
  purchaseBills: {
    view: "purchaseBills.view",
    create: "purchaseBills.create",
    update: "purchaseBills.update",
    finalize: "purchaseBills.finalize",
    void: "purchaseBills.void",
  },
  purchaseOrders: {
    view: "purchaseOrders.view",
    create: "purchaseOrders.create",
    update: "purchaseOrders.update",
    approve: "purchaseOrders.approve",
    void: "purchaseOrders.void",
    convertToBill: "purchaseOrders.convertToBill",
    send: "purchaseOrders.send",
  },
  supplierPayments: {
    view: "supplierPayments.view",
    create: "supplierPayments.create",
    void: "supplierPayments.void",
    send: "supplierPayments.send",
  },
  purchaseDebitNotes: {
    view: "purchaseDebitNotes.view",
    create: "purchaseDebitNotes.create",
    finalize: "purchaseDebitNotes.finalize",
    void: "purchaseDebitNotes.void",
    send: "purchaseDebitNotes.send",
  },
  supplierRefunds: {
    view: "supplierRefunds.view",
    create: "supplierRefunds.create",
    void: "supplierRefunds.void",
  },
  cashExpenses: {
    view: "cashExpenses.view",
    create: "cashExpenses.create",
    void: "cashExpenses.void",
  },
  recurringTransactions: {
    read: "recurringTransactions.read",
    manage: "recurringTransactions.manage",
    run: "recurringTransactions.run",
    review: "recurringTransactions.review",
  },
  reports: {
    view: "reports.view",
    export: "reports.export",
  },
  documents: {
    view: "documents.view",
    download: "documents.download",
  },
  documentSettings: {
    view: "documentSettings.view",
    manage: "documentSettings.manage",
  },
  numberSequences: {
    view: "numberSequences.view",
    manage: "numberSequences.manage",
  },
  compliance: {
    view: "compliance.view",
    manage: "compliance.manage",
    validate: "compliance.validate",
    archive: "compliance.archive",
  },
  zatca: {
    view: "zatca.view",
    manage: "zatca.manage",
    generateXml: "zatca.generateXml",
    runChecks: "zatca.runChecks",
    signingDryRun: "zatca.signing.dryRun",
    submit: "zatca.submit",
  },
  generatedDocuments: {
    view: "generatedDocuments.view",
    download: "generatedDocuments.download",
  },
  documentInbox: {
    view: "documentInbox.view",
    upload: "documentInbox.upload",
    review: "documentInbox.review",
  },
  payments: {
    providerReadinessView: "payments.providerReadiness.view",
    paymentLinksCreate: "payments.paymentLinks.create",
    providerEventsManage: "payments.providerEvents.manage",
  },
  attachments: {
    view: "attachments.view",
    upload: "attachments.upload",
    download: "attachments.download",
    delete: "attachments.delete",
    manage: "attachments.manage",
  },
  emailOutbox: {
    view: "emailOutbox.view",
  },
  migrationToolkit: {
    view: "migrationToolkit.view",
    import: "migrationToolkit.import",
    commit: "migrationToolkit.commit",
    export: "migrationToolkit.export",
  },
  auditLogs: {
    view: "auditLogs.view",
    export: "auditLogs.export",
    manageRetention: "auditLogs.manageRetention",
  },
  admin: {
    fullAccess: "admin.fullAccess",
  },
} as const;

type DeepValue<T> = T extends string ? T : T[keyof T] extends infer Value ? DeepValue<Value> : never;

export type Permission = DeepValue<typeof PERMISSIONS>;

export const ALL_PERMISSIONS = [
  PERMISSIONS.dashboard.view,
  PERMISSIONS.organization.view,
  PERMISSIONS.organization.update,
  PERMISSIONS.users.view,
  PERMISSIONS.users.invite,
  PERMISSIONS.users.manage,
  PERMISSIONS.roles.view,
  PERMISSIONS.roles.manage,
  PERMISSIONS.accounts.view,
  PERMISSIONS.accounts.manage,
  PERMISSIONS.currencies.read,
  PERMISSIONS.currencies.manage,
  PERMISSIONS.fxRates.read,
  PERMISSIONS.fxRates.manage,
  PERMISSIONS.fxRevaluation.read,
  PERMISSIONS.fxRevaluation.run,
  PERMISSIONS.fxRevaluation.reverse,
  PERMISSIONS.bankAccounts.view,
  PERMISSIONS.bankAccounts.manage,
  PERMISSIONS.bankAccounts.transactionsView,
  PERMISSIONS.bankAccounts.openingBalancePost,
  PERMISSIONS.bankTransfers.view,
  PERMISSIONS.bankTransfers.create,
  PERMISSIONS.bankTransfers.void,
  PERMISSIONS.bankStatements.view,
  PERMISSIONS.bankStatements.import,
  PERMISSIONS.bankStatements.previewImport,
  PERMISSIONS.bankStatements.reconcile,
  PERMISSIONS.bankStatements.manage,
  PERMISSIONS.bankReconciliations.view,
  PERMISSIONS.bankReconciliations.create,
  PERMISSIONS.bankReconciliations.approve,
  PERMISSIONS.bankReconciliations.reopen,
  PERMISSIONS.bankReconciliations.close,
  PERMISSIONS.bankReconciliations.void,
  PERMISSIONS.bankIntegrations.connectionManage,
  PERMISSIONS.bankIntegrations.feedRead,
  PERMISSIONS.bankIntegrations.beneficiaryManage,
  PERMISSIONS.bankIntegrations.vendorPaymentCreate,
  PERMISSIONS.bankIntegrations.vendorPaymentApprove,
  PERMISSIONS.bankIntegrations.vendorPaymentReconcile,
  PERMISSIONS.taxRates.view,
  PERMISSIONS.taxRates.manage,
  PERMISSIONS.journals.view,
  PERMISSIONS.journals.create,
  PERMISSIONS.journals.post,
  PERMISSIONS.journals.reverse,
  PERMISSIONS.fixedAssets.read,
  PERMISSIONS.fixedAssets.manage,
  PERMISSIONS.fixedAssets.categoriesManage,
  PERMISSIONS.fixedAssets.capitalize,
  PERMISSIONS.fixedAssets.depreciationReview,
  PERMISSIONS.fixedAssets.depreciationPost,
  PERMISSIONS.fixedAssets.dispose,
  PERMISSIONS.fixedAssets.reports,
  PERMISSIONS.fiscalPeriods.view,
  PERMISSIONS.fiscalPeriods.manage,
  PERMISSIONS.fiscalPeriods.lock,
  PERMISSIONS.accountingClose.read,
  PERMISSIONS.accountingClose.manage,
  PERMISSIONS.accountingClose.prepare,
  PERMISSIONS.accountingClose.review,
  PERMISSIONS.accountingClose.close,
  PERMISSIONS.accountingClose.lock,
  PERMISSIONS.contacts.view,
  PERMISSIONS.contacts.manage,
  PERMISSIONS.contacts.sendCustomerStatements,
  PERMISSIONS.contacts.sendSupplierStatements,
  PERMISSIONS.items.view,
  PERMISSIONS.items.manage,
  PERMISSIONS.inventory.view,
  PERMISSIONS.inventory.manage,
  PERMISSIONS.inventory.cogsPost,
  PERMISSIONS.inventory.cogsReverse,
  PERMISSIONS.inventory.receiptsPostAsset,
  PERMISSIONS.inventory.receiptsReverseAsset,
  PERMISSIONS.inventory.varianceProposalsView,
  PERMISSIONS.inventory.varianceProposalsCreate,
  PERMISSIONS.inventory.varianceProposalsApprove,
  PERMISSIONS.inventory.varianceProposalsPost,
  PERMISSIONS.inventory.varianceProposalsReverse,
  PERMISSIONS.inventory.varianceProposalsVoid,
  PERMISSIONS.warehouses.view,
  PERMISSIONS.warehouses.manage,
  PERMISSIONS.stockMovements.view,
  PERMISSIONS.stockMovements.create,
  PERMISSIONS.stockAdjustments.create,
  PERMISSIONS.inventoryAdjustments.view,
  PERMISSIONS.inventoryAdjustments.create,
  PERMISSIONS.inventoryAdjustments.approve,
  PERMISSIONS.inventoryAdjustments.void,
  PERMISSIONS.warehouseTransfers.view,
  PERMISSIONS.warehouseTransfers.create,
  PERMISSIONS.warehouseTransfers.void,
  PERMISSIONS.purchaseReceiving.view,
  PERMISSIONS.purchaseReceiving.create,
  PERMISSIONS.salesStockIssue.view,
  PERMISSIONS.salesStockIssue.create,
  PERMISSIONS.salesInvoices.view,
  PERMISSIONS.salesInvoices.create,
  PERMISSIONS.salesInvoices.update,
  PERMISSIONS.salesInvoices.finalize,
  PERMISSIONS.salesInvoices.void,
  PERMISSIONS.salesInvoices.send,
  PERMISSIONS.customerPayments.view,
  PERMISSIONS.customerPayments.create,
  PERMISSIONS.customerPayments.applyUnapplied,
  PERMISSIONS.customerPayments.reverseUnappliedAllocation,
  PERMISSIONS.customerPayments.receiptPdfGenerate,
  PERMISSIONS.customerPayments.send,
  PERMISSIONS.customerPayments.void,
  PERMISSIONS.creditNotes.view,
  PERMISSIONS.creditNotes.create,
  PERMISSIONS.creditNotes.finalize,
  PERMISSIONS.creditNotes.send,
  PERMISSIONS.creditNotes.void,
  PERMISSIONS.customerRefunds.view,
  PERMISSIONS.customerRefunds.create,
  PERMISSIONS.customerRefunds.void,
  PERMISSIONS.purchaseBills.view,
  PERMISSIONS.purchaseBills.create,
  PERMISSIONS.purchaseBills.update,
  PERMISSIONS.purchaseBills.finalize,
  PERMISSIONS.purchaseBills.void,
  PERMISSIONS.purchaseOrders.view,
  PERMISSIONS.purchaseOrders.create,
  PERMISSIONS.purchaseOrders.update,
  PERMISSIONS.purchaseOrders.approve,
  PERMISSIONS.purchaseOrders.void,
  PERMISSIONS.purchaseOrders.convertToBill,
  PERMISSIONS.purchaseOrders.send,
  PERMISSIONS.supplierPayments.view,
  PERMISSIONS.supplierPayments.create,
  PERMISSIONS.supplierPayments.void,
  PERMISSIONS.supplierPayments.send,
  PERMISSIONS.purchaseDebitNotes.view,
  PERMISSIONS.purchaseDebitNotes.create,
  PERMISSIONS.purchaseDebitNotes.finalize,
  PERMISSIONS.purchaseDebitNotes.void,
  PERMISSIONS.purchaseDebitNotes.send,
  PERMISSIONS.supplierRefunds.view,
  PERMISSIONS.supplierRefunds.create,
  PERMISSIONS.supplierRefunds.void,
  PERMISSIONS.cashExpenses.view,
  PERMISSIONS.cashExpenses.create,
  PERMISSIONS.cashExpenses.void,
  PERMISSIONS.recurringTransactions.read,
  PERMISSIONS.recurringTransactions.manage,
  PERMISSIONS.recurringTransactions.run,
  PERMISSIONS.recurringTransactions.review,
  PERMISSIONS.reports.view,
  PERMISSIONS.reports.export,
  PERMISSIONS.documents.view,
  PERMISSIONS.documents.download,
  PERMISSIONS.documentSettings.view,
  PERMISSIONS.documentSettings.manage,
  PERMISSIONS.numberSequences.view,
  PERMISSIONS.numberSequences.manage,
  PERMISSIONS.compliance.view,
  PERMISSIONS.compliance.manage,
  PERMISSIONS.compliance.validate,
  PERMISSIONS.compliance.archive,
  PERMISSIONS.zatca.view,
  PERMISSIONS.zatca.manage,
  PERMISSIONS.zatca.generateXml,
  PERMISSIONS.zatca.runChecks,
  PERMISSIONS.zatca.signingDryRun,
  PERMISSIONS.zatca.submit,
  PERMISSIONS.generatedDocuments.view,
  PERMISSIONS.generatedDocuments.download,
  PERMISSIONS.documentInbox.view,
  PERMISSIONS.documentInbox.upload,
  PERMISSIONS.documentInbox.review,
  PERMISSIONS.payments.providerReadinessView,
  PERMISSIONS.payments.paymentLinksCreate,
  PERMISSIONS.payments.providerEventsManage,
  PERMISSIONS.attachments.view,
  PERMISSIONS.attachments.upload,
  PERMISSIONS.attachments.download,
  PERMISSIONS.attachments.delete,
  PERMISSIONS.attachments.manage,
  PERMISSIONS.emailOutbox.view,
  PERMISSIONS.migrationToolkit.view,
  PERMISSIONS.migrationToolkit.import,
  PERMISSIONS.migrationToolkit.commit,
  PERMISSIONS.migrationToolkit.export,
  PERMISSIONS.auditLogs.view,
  PERMISSIONS.auditLogs.export,
  PERMISSIONS.auditLogs.manageRetention,
  PERMISSIONS.admin.fullAccess,
] as const satisfies readonly Permission[];

export const LEGACY_FULL_ACCESS_PERMISSION = "*";

const ADMIN_ROLE_PERMISSIONS = ALL_PERMISSIONS.filter((permission) => permission !== PERMISSIONS.admin.fullAccess);

const ACCOUNTANT_ROLE_PERMISSIONS = [
  PERMISSIONS.dashboard.view,
  PERMISSIONS.organization.view,
  PERMISSIONS.accounts.view,
  PERMISSIONS.accounts.manage,
  PERMISSIONS.currencies.read,
  PERMISSIONS.currencies.manage,
  PERMISSIONS.fxRates.read,
  PERMISSIONS.fxRates.manage,
  PERMISSIONS.fxRevaluation.read,
  PERMISSIONS.fxRevaluation.run,
  PERMISSIONS.fxRevaluation.reverse,
  PERMISSIONS.bankAccounts.view,
  PERMISSIONS.bankAccounts.manage,
  PERMISSIONS.bankAccounts.transactionsView,
  PERMISSIONS.bankAccounts.openingBalancePost,
  PERMISSIONS.bankTransfers.view,
  PERMISSIONS.bankTransfers.create,
  PERMISSIONS.bankTransfers.void,
  PERMISSIONS.bankStatements.view,
  PERMISSIONS.bankStatements.import,
  PERMISSIONS.bankStatements.previewImport,
  PERMISSIONS.bankStatements.reconcile,
  PERMISSIONS.bankStatements.manage,
  PERMISSIONS.bankReconciliations.view,
  PERMISSIONS.bankReconciliations.create,
  PERMISSIONS.bankReconciliations.approve,
  PERMISSIONS.bankReconciliations.reopen,
  PERMISSIONS.bankReconciliations.close,
  PERMISSIONS.bankReconciliations.void,
  PERMISSIONS.bankIntegrations.connectionManage,
  PERMISSIONS.bankIntegrations.feedRead,
  PERMISSIONS.bankIntegrations.beneficiaryManage,
  PERMISSIONS.bankIntegrations.vendorPaymentCreate,
  PERMISSIONS.bankIntegrations.vendorPaymentApprove,
  PERMISSIONS.bankIntegrations.vendorPaymentReconcile,
  PERMISSIONS.taxRates.view,
  PERMISSIONS.taxRates.manage,
  PERMISSIONS.journals.view,
  PERMISSIONS.journals.create,
  PERMISSIONS.journals.post,
  PERMISSIONS.journals.reverse,
  PERMISSIONS.fixedAssets.read,
  PERMISSIONS.fixedAssets.manage,
  PERMISSIONS.fixedAssets.categoriesManage,
  PERMISSIONS.fixedAssets.capitalize,
  PERMISSIONS.fixedAssets.depreciationReview,
  PERMISSIONS.fixedAssets.depreciationPost,
  PERMISSIONS.fixedAssets.dispose,
  PERMISSIONS.fixedAssets.reports,
  PERMISSIONS.fiscalPeriods.view,
  PERMISSIONS.fiscalPeriods.manage,
  PERMISSIONS.accountingClose.read,
  PERMISSIONS.accountingClose.manage,
  PERMISSIONS.accountingClose.prepare,
  PERMISSIONS.accountingClose.review,
  PERMISSIONS.accountingClose.close,
  PERMISSIONS.contacts.view,
  PERMISSIONS.contacts.sendCustomerStatements,
  PERMISSIONS.contacts.sendSupplierStatements,
  PERMISSIONS.items.view,
  PERMISSIONS.inventory.view,
  PERMISSIONS.inventory.manage,
  PERMISSIONS.inventory.cogsPost,
  PERMISSIONS.inventory.cogsReverse,
  PERMISSIONS.inventory.receiptsPostAsset,
  PERMISSIONS.inventory.receiptsReverseAsset,
  PERMISSIONS.inventory.varianceProposalsView,
  PERMISSIONS.inventory.varianceProposalsCreate,
  PERMISSIONS.inventory.varianceProposalsApprove,
  PERMISSIONS.inventory.varianceProposalsPost,
  PERMISSIONS.inventory.varianceProposalsReverse,
  PERMISSIONS.inventory.varianceProposalsVoid,
  PERMISSIONS.warehouses.view,
  PERMISSIONS.warehouses.manage,
  PERMISSIONS.stockMovements.view,
  PERMISSIONS.stockMovements.create,
  PERMISSIONS.stockAdjustments.create,
  PERMISSIONS.inventoryAdjustments.view,
  PERMISSIONS.inventoryAdjustments.create,
  PERMISSIONS.inventoryAdjustments.approve,
  PERMISSIONS.inventoryAdjustments.void,
  PERMISSIONS.warehouseTransfers.view,
  PERMISSIONS.warehouseTransfers.create,
  PERMISSIONS.warehouseTransfers.void,
  PERMISSIONS.purchaseReceiving.view,
  PERMISSIONS.purchaseReceiving.create,
  PERMISSIONS.salesStockIssue.view,
  PERMISSIONS.salesStockIssue.create,
  PERMISSIONS.salesInvoices.view,
  PERMISSIONS.salesInvoices.finalize,
  PERMISSIONS.salesInvoices.void,
  PERMISSIONS.salesInvoices.send,
  PERMISSIONS.customerPayments.view,
  PERMISSIONS.customerPayments.create,
  PERMISSIONS.customerPayments.applyUnapplied,
  PERMISSIONS.customerPayments.reverseUnappliedAllocation,
  PERMISSIONS.customerPayments.receiptPdfGenerate,
  PERMISSIONS.customerPayments.send,
  PERMISSIONS.customerPayments.void,
  PERMISSIONS.creditNotes.view,
  PERMISSIONS.creditNotes.create,
  PERMISSIONS.creditNotes.finalize,
  PERMISSIONS.creditNotes.send,
  PERMISSIONS.creditNotes.void,
  PERMISSIONS.customerRefunds.view,
  PERMISSIONS.customerRefunds.create,
  PERMISSIONS.customerRefunds.void,
  PERMISSIONS.purchaseBills.view,
  PERMISSIONS.purchaseBills.finalize,
  PERMISSIONS.purchaseBills.void,
  PERMISSIONS.purchaseOrders.view,
  PERMISSIONS.purchaseOrders.create,
  PERMISSIONS.purchaseOrders.update,
  PERMISSIONS.purchaseOrders.approve,
  PERMISSIONS.purchaseOrders.void,
  PERMISSIONS.purchaseOrders.convertToBill,
  PERMISSIONS.purchaseOrders.send,
  PERMISSIONS.supplierPayments.view,
  PERMISSIONS.supplierPayments.create,
  PERMISSIONS.supplierPayments.void,
  PERMISSIONS.supplierPayments.send,
  PERMISSIONS.purchaseDebitNotes.view,
  PERMISSIONS.purchaseDebitNotes.create,
  PERMISSIONS.purchaseDebitNotes.finalize,
  PERMISSIONS.purchaseDebitNotes.void,
  PERMISSIONS.purchaseDebitNotes.send,
  PERMISSIONS.supplierRefunds.view,
  PERMISSIONS.supplierRefunds.create,
  PERMISSIONS.supplierRefunds.void,
  PERMISSIONS.cashExpenses.view,
  PERMISSIONS.cashExpenses.create,
  PERMISSIONS.cashExpenses.void,
  PERMISSIONS.recurringTransactions.read,
  PERMISSIONS.recurringTransactions.manage,
  PERMISSIONS.recurringTransactions.run,
  PERMISSIONS.recurringTransactions.review,
  PERMISSIONS.reports.view,
  PERMISSIONS.reports.export,
  PERMISSIONS.documents.view,
  PERMISSIONS.documents.download,
  PERMISSIONS.documentSettings.view,
  PERMISSIONS.generatedDocuments.view,
  PERMISSIONS.generatedDocuments.download,
  PERMISSIONS.documentInbox.view,
  PERMISSIONS.documentInbox.upload,
  PERMISSIONS.documentInbox.review,
  PERMISSIONS.payments.providerReadinessView,
  PERMISSIONS.payments.paymentLinksCreate,
  PERMISSIONS.attachments.view,
  PERMISSIONS.attachments.upload,
  PERMISSIONS.attachments.download,
  PERMISSIONS.attachments.delete,
  PERMISSIONS.attachments.manage,
  PERMISSIONS.auditLogs.view,
  PERMISSIONS.auditLogs.export,
  PERMISSIONS.numberSequences.view,
  PERMISSIONS.numberSequences.manage,
  PERMISSIONS.compliance.view,
  PERMISSIONS.compliance.validate,
  PERMISSIONS.compliance.archive,
  PERMISSIONS.zatca.view,
  PERMISSIONS.zatca.generateXml,
  PERMISSIONS.zatca.runChecks,
] as const satisfies readonly Permission[];

const SALES_ROLE_PERMISSIONS = [
  PERMISSIONS.dashboard.view,
  PERMISSIONS.organization.view,
  PERMISSIONS.currencies.read,
  PERMISSIONS.fxRates.read,
  PERMISSIONS.contacts.view,
  PERMISSIONS.contacts.manage,
  PERMISSIONS.contacts.sendCustomerStatements,
  PERMISSIONS.items.view,
  PERMISSIONS.inventory.view,
  PERMISSIONS.warehouses.view,
  PERMISSIONS.salesStockIssue.view,
  PERMISSIONS.salesStockIssue.create,
  PERMISSIONS.salesInvoices.view,
  PERMISSIONS.salesInvoices.create,
  PERMISSIONS.salesInvoices.update,
  PERMISSIONS.salesInvoices.send,
  PERMISSIONS.customerPayments.view,
  PERMISSIONS.customerPayments.create,
  PERMISSIONS.customerPayments.applyUnapplied,
  PERMISSIONS.customerPayments.receiptPdfGenerate,
  PERMISSIONS.customerPayments.send,
  PERMISSIONS.creditNotes.view,
  PERMISSIONS.creditNotes.create,
  PERMISSIONS.creditNotes.send,
  PERMISSIONS.customerRefunds.view,
  PERMISSIONS.documents.view,
  PERMISSIONS.documents.download,
  PERMISSIONS.generatedDocuments.view,
  PERMISSIONS.generatedDocuments.download,
  PERMISSIONS.payments.providerReadinessView,
  PERMISSIONS.payments.paymentLinksCreate,
  PERMISSIONS.attachments.view,
  PERMISSIONS.attachments.upload,
  PERMISSIONS.attachments.download,
] as const satisfies readonly Permission[];

const PURCHASES_ROLE_PERMISSIONS = [
  PERMISSIONS.dashboard.view,
  PERMISSIONS.organization.view,
  PERMISSIONS.currencies.read,
  PERMISSIONS.fxRates.read,
  PERMISSIONS.contacts.view,
  PERMISSIONS.contacts.manage,
  PERMISSIONS.contacts.sendSupplierStatements,
  PERMISSIONS.items.view,
  PERMISSIONS.inventory.view,
  PERMISSIONS.inventory.varianceProposalsView,
  PERMISSIONS.inventory.varianceProposalsCreate,
  PERMISSIONS.warehouses.view,
  PERMISSIONS.stockMovements.view,
  PERMISSIONS.inventoryAdjustments.view,
  PERMISSIONS.inventoryAdjustments.create,
  PERMISSIONS.warehouseTransfers.view,
  PERMISSIONS.warehouseTransfers.create,
  PERMISSIONS.purchaseReceiving.view,
  PERMISSIONS.purchaseReceiving.create,
  PERMISSIONS.bankAccounts.view,
  PERMISSIONS.bankAccounts.transactionsView,
  PERMISSIONS.bankIntegrations.feedRead,
  PERMISSIONS.bankIntegrations.beneficiaryManage,
  PERMISSIONS.bankIntegrations.vendorPaymentCreate,
  PERMISSIONS.purchaseOrders.view,
  PERMISSIONS.purchaseOrders.create,
  PERMISSIONS.purchaseOrders.update,
  PERMISSIONS.purchaseOrders.approve,
  PERMISSIONS.purchaseOrders.void,
  PERMISSIONS.purchaseOrders.convertToBill,
  PERMISSIONS.purchaseOrders.send,
  PERMISSIONS.purchaseBills.view,
  PERMISSIONS.purchaseBills.create,
  PERMISSIONS.purchaseBills.update,
  PERMISSIONS.supplierPayments.view,
  PERMISSIONS.supplierPayments.create,
  PERMISSIONS.supplierPayments.send,
  PERMISSIONS.purchaseDebitNotes.view,
  PERMISSIONS.purchaseDebitNotes.create,
  PERMISSIONS.purchaseDebitNotes.send,
  PERMISSIONS.supplierRefunds.view,
  PERMISSIONS.cashExpenses.view,
  PERMISSIONS.cashExpenses.create,
  PERMISSIONS.documents.view,
  PERMISSIONS.documents.download,
  PERMISSIONS.generatedDocuments.view,
  PERMISSIONS.generatedDocuments.download,
  PERMISSIONS.documentInbox.view,
  PERMISSIONS.documentInbox.upload,
  PERMISSIONS.documentInbox.review,
  PERMISSIONS.attachments.view,
  PERMISSIONS.attachments.upload,
  PERMISSIONS.attachments.download,
] as const satisfies readonly Permission[];

const VIEWER_ROLE_PERMISSIONS = [
  PERMISSIONS.dashboard.view,
  PERMISSIONS.organization.view,
  PERMISSIONS.accounts.view,
  PERMISSIONS.currencies.read,
  PERMISSIONS.fxRates.read,
  PERMISSIONS.fxRevaluation.read,
  PERMISSIONS.taxRates.view,
  PERMISSIONS.journals.view,
  PERMISSIONS.fixedAssets.read,
  PERMISSIONS.fixedAssets.reports,
  PERMISSIONS.fiscalPeriods.view,
  PERMISSIONS.contacts.view,
  PERMISSIONS.items.view,
  PERMISSIONS.inventory.view,
  PERMISSIONS.inventory.varianceProposalsView,
  PERMISSIONS.warehouses.view,
  PERMISSIONS.stockMovements.view,
  PERMISSIONS.inventoryAdjustments.view,
  PERMISSIONS.warehouseTransfers.view,
  PERMISSIONS.purchaseReceiving.view,
  PERMISSIONS.salesStockIssue.view,
  PERMISSIONS.salesInvoices.view,
  PERMISSIONS.customerPayments.view,
  PERMISSIONS.creditNotes.view,
  PERMISSIONS.customerRefunds.view,
  PERMISSIONS.purchaseOrders.view,
  PERMISSIONS.purchaseBills.view,
  PERMISSIONS.supplierPayments.view,
  PERMISSIONS.purchaseDebitNotes.view,
  PERMISSIONS.supplierRefunds.view,
  PERMISSIONS.cashExpenses.view,
  PERMISSIONS.recurringTransactions.read,
  PERMISSIONS.reports.view,
  PERMISSIONS.documents.view,
  PERMISSIONS.documents.download,
  PERMISSIONS.documentSettings.view,
  PERMISSIONS.generatedDocuments.view,
  PERMISSIONS.generatedDocuments.download,
  PERMISSIONS.documentInbox.view,
  PERMISSIONS.bankIntegrations.feedRead,
  PERMISSIONS.attachments.view,
  PERMISSIONS.attachments.download,
  PERMISSIONS.compliance.view,
  PERMISSIONS.zatca.view,
] as const satisfies readonly Permission[];

export const DEFAULT_ROLE_PERMISSIONS = {
  Owner: ALL_PERMISSIONS,
  Admin: ADMIN_ROLE_PERMISSIONS,
  Accountant: ACCOUNTANT_ROLE_PERMISSIONS,
  Sales: SALES_ROLE_PERMISSIONS,
  Purchases: PURCHASES_ROLE_PERMISSIONS,
  Viewer: VIEWER_ROLE_PERMISSIONS,
} as const satisfies Record<string, readonly Permission[]>;

export type DefaultRoleName = keyof typeof DEFAULT_ROLE_PERMISSIONS;

export function normalizePermissions(value: unknown): string[] {
  if (Array.isArray(value)) {
    return value.filter((permission): permission is string => typeof permission === "string");
  }

  if (isRecord(value) && Array.isArray(value.permissions)) {
    return value.permissions.filter((permission): permission is string => typeof permission === "string");
  }

  return [];
}

export function isFullAccess(permissions: unknown): boolean {
  const normalized = normalizePermissions(permissions);
  return normalized.includes(PERMISSIONS.admin.fullAccess) || normalized.includes(LEGACY_FULL_ACCESS_PERMISSION);
}

export function hasPermission(permissions: unknown, permission: Permission | string): boolean {
  const normalized = normalizePermissions(permissions);
  return isFullAccess(normalized) || normalized.includes(permission);
}

export function hasAnyPermission(permissions: unknown, requiredPermissions: readonly (Permission | string)[]): boolean {
  return requiredPermissions.length === 0 || requiredPermissions.some((permission) => hasPermission(permissions, permission));
}

export function hasAllPermissions(permissions: unknown, requiredPermissions: readonly (Permission | string)[]): boolean {
  return requiredPermissions.every((permission) => hasPermission(permissions, permission));
}

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null;
}
