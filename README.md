...
- Latest local fresh-EGS result: invoice 1 stored SDK hash `3G0f1iTuJNYnHJY8dJWsoGfz9jfCBaTwNb+UK84ILaU=` using the official first PIH seed; invoice 2 stored SDK hash `Eoo9jY0Tcf1zof/rjR3LPIXXsyxnLNvzrIcZLR9OczY=` using invoice 1's SDK hash as PIH; both hash-compare calls returned `MATCH`; repeat generation was idempotent. Invoice 2 still fails official SDK XML validation with `KSA-13`, and generated buyer-address warnings remain.
