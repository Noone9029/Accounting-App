...
- Real ZATCA production compliance is not enabled. Local generated standard invoice and credit-note fixtures passed no-network SDK validation under Java 11.0.26, the approved local dummy-material run passed sign/QR/signed-validation for sanitized fixtures, and key custody/CSID lifecycle design is documented in `docs/zatca/KEY_CUSTODY_AND_CSID_LIFECYCLE_DESIGN.md`, `docs/zatca/CSID_LIFECYCLE_CHECKLIST.md`, and `docs/zatca/KEY_CUSTODY_DECISION_MATRIX.md`. SDK CI readiness is currently `CI_BLOCKED_MISSING_SDK_REFERENCE`. CSID execution, production signing, production Phase 2 QR, clearance/reporting, PDF/A-3, real network submission, and production compliance certification remain blocked.
