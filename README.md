...
2026-05-20 narrow deployed phase validation note: after splitting the tail further, `smoke:accounting:ar`, `smoke:accounting:ap`, and `smoke:accounting:documents` passed with secret-store credentials and generated-user fallback unset. `smoke:accounting:reports` stopped on the first per-request timeout at `GET /dashboard/summary` after 60 seconds, after the report endpoints had completed. API/web health remained HTTP `200`, readiness stayed `ok`, and pool counts stayed stable from `active=1`, `idle=5`, `unknown=8` before to `active=1`, `idle=7`, `unknown=8` after. `smoke:accounting:zatca-safe`, full smoke, and full E2E were intentionally deferred by the stop rule. The next smoke task should diagnose the dashboard summary deployed latency specifically; do not restart generic smoke splitting unless that route diagnosis shows a phase-shape issue.
