...
- Real ZATCA production compliance is not enabled. Local generated standard invoice and credit-note fixtures passed no-network SDK validation under Java 11.0.26, SDK CI readiness is currently `CI_BLOCKED_MISSING_SDK_REFERENCE`, the local signed XML validation plan guard is blocked/planning-only, and the dummy signing dry-run guard is disabled by default. CSID execution, signing, QR generation, signed XML validation, clearance/reporting, PDF/A-3, real network submission, and production compliance certification remain blocked.
