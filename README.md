...
- DEV-09 local banking/reconciliation evidence is closed for its local-only scope. Wafeq-style manual import UX now includes a downloadable CSV template and XLSX server-preview/import support, but banking is not production-complete and remains unproven for live bank feeds, automatic matching, certified bank-specific parser coverage, raw statement archive operations, production/beta/customer data, broad E2E/smoke/full-test coverage, and accountant sign-off.
...
For deployed smoke diagnostics, set `LEDGERBYTE_SMOKE_REQUEST_TIMEOUT_MS=60000` and `LEDGERBYTE_SMOKE_PROGRESS=true` to add request-level timeouts and redacted method/path/status progress. Progress output must not include passwords, tokens, headers, connection strings, request bodies, response bodies, customer document bodies, or attachment bodies.

To isolate the deployed banking workflow without running the full accounting smoke, use:
...
To isolate the late deployed accounting tail without running the full accounting smoke, use the aggregate tail command or one of the narrow deployed phases:
...
The aggregate tail phase runs the narrow phases in order. The narrow phases independently create their own smoke records: `ar` covers customer invoices, payments, overpayments, refunds, credit notes, customer ledger/statement checks, payment void checks, and active-payment invoice-void blocking; `ap` covers purchase bills, debit notes, supplier payments/refunds, supplier ledgers/statements, and payment detail reads; `documents` covers document settings, generated document archive downloads, attachments, representative audit log redaction, storage readiness, migration-plan dry runs, and backup/restore readiness planning without executing backups or restores; `reports` covers report CSV/PDF availability, aging reports, dashboard summary, and dashboard onboarding checks; `zatca-safe` covers safe no-network ZATCA readiness/checklist/XML/SDK/mock-blocking checks with `productionCompliance=false`. The full smoke command remains available for complete local or long-ceiling validation.

2026-05-20 deployed validation note: the first `smoke:accounting:tail` run against Git-triggered commit `1b7ff0dbbc331c3f1b721ace29ce2a562c6f381d` reached the 45-minute hard ceiling while still making forward progress. It had no per-route 60-second timeout, no smoke-fetch error, no stderr output, stable pool counts, and healthy API/web checks afterward. Split the late AP/supplier-payment/reporting remainder further before using tail completion as a gate for full deployed smoke or browser E2E.

2026-05-20 narrow deployed phase validation note: after splitting the tail further, `smoke:accounting:ar`, `smoke:accounting:ap`, and `smoke:accounting:documents` passed with secret-store credentials and generated-user fallback unset. The first `smoke:accounting:reports` run stopped on the first per-request timeout at `GET /dashboard/summary` after 60 seconds, after the report endpoints had completed. API/web health remained HTTP `200`, readiness stayed `ok`, and pool counts stayed stable from `active=1`, `idle=5`, `unknown=8` before to `active=1`, `idle=7`, `unknown=8` after.

2026-05-20 dashboard-summary latency fix note: commit `b00f7897b8ffdf887826f79e626b08a09cabbcee` replaced dashboard-only report/trend summary calls with tenant-scoped summary journal-line aggregation, avoiding repeated full report helper calls inside `GET /dashboard/summary` while preserving the public dashboard response shape. Git-triggered deployments `dpl_WZHz5HbibMnGTpUinQQXYy23K73R` (API) and `dpl_4EGbTS4Tc3CCGRjTY4s7n3pP8A22` (web) served the fix. `smoke:accounting:reports` then passed with `GET /dashboard/summary -> 200` in 45,489 ms, and `smoke:accounting:zatca-safe` passed with ZATCA production compliance false and real ZATCA network disabled. Pre/post health stayed HTTP `200`; pool counts stayed stable from `active=1`, `idle=5`, `unknown=2` before to `active=1`, `idle=7`, `unknown=2` after. Full smoke and full E2E remain intentionally pending; the next project task should be Supabase RLS/Data API/least-privilege runtime DB hardening, not more generic smoke splitting.

2026-05-21 Supabase Data API hardening note: the user-testing Supabase project still has 76 public tables with RLS disabled, so broad RLS was not enabled blindly. The web app was confirmed to use the Nest API only, with no direct Supabase REST/GraphQL/Storage client usage found. As an immediate safe mitigation, `anon` and `authenticated` grants were revoked from public tables, sequences, and functions in user-testing, and `postgres`-owned future default public grants for those roles were revoked. API/web health remained HTTP `200`, readiness stayed `ok`, and `smoke:accounting:reports` passed with secret-store credentials. The Data API Dashboard toggle and a least-privilege Prisma runtime role remain follow-up security tasks.

2026-05-21 Prisma runtime role hardening note: the planned user-testing-only runtime role is `ledgerbyte_app_runtime_user_testing`, with no superuser, createdb, createrole, replication, or bypassrls privileges, public app-table DML only, no `_prisma_migrations` DML, and `DIRECT_URL` kept on the migration/admin credential. The role was not created because this session had no safe Vercel API `DATABASE_URL` mutation path to store the generated password, redeploy, and validate without printing secrets. Current direct `anon`/`authenticated` object grants remain revoked; `supabase_admin` future-object default privileges still need a reviewed follow-up pass.

2026-05-21 first-workflow UX note: the setup wizard and dashboard now guide a new business user through a real first accounting loop: complete profile and VAT/tax details, add a first customer, create/finalize a first sales invoice, record a first customer payment, and open the first report. The checklist is based on real tenant data from `GET /dashboard/onboarding-checklist`, adds first-payment and first-reportable-activity signals, and keeps ZATCA messaging safe by warning that production ZATCA is not connected yet.
...
- `POST /email/suppressions/:id/revoke`
...
- Manual statement import now supports upload or paste of CSV/JSON/text plus limited OFX, CAMT XML, MT940, and XLSX exports, with a 1 MB file limit, local row preview for text formats, server preview for XLSX, detected columns, duplicate-candidate warnings, and validation guidance before import.
- CSV/JSON/OFX/CAMT/MT940/XLSX preview/import supports common fields including `date`, `transactionDate`, `postedDate`, `description`, `memo`, `details`, `reference`, `bankReference`, `debit`, `credit`, `amount`, `balance`, `counterparty`, and `currency` where those fields are present in the manual file. OFX XML-style transactions, missing FITID warnings, CAMT.053/CAMT.054 entry direction/date/reference fallbacks, MT940 comma-decimal/multiline `:86:` narratives, and first-worksheet XLSX canonical templates have targeted sanitized fixture coverage.
- The statement import page provides a downloadable CSV template with canonical columns `date`, `description`, `reference`, `bankReference`, `debit`, `credit`, `amount`, `balance`, `counterparty`, and `currency`. The same columns can be used in the first worksheet of an XLSX workbook.
...
- Retry processor configuration: `LEDGERBYTE_EMAIL_RETRY_PROCESSOR_ENABLED=false` by default.
...
- `POST /email/provider-events/mock` captures metadata-only local/mock delivery events, rejects secrets and customer message content, and never marks production monitoring ready because signatures are not verified.
- `GET /email/suppressions`, `POST /email/suppressions`, and `POST /email/suppressions/:id/revoke` manage tenant-scoped suppression metadata. Manual suppression accepts an email for matching but stores and returns only a hash plus masked address; active suppressions block future matched send/retry attempts.
...
- `GET /email/readiness` is an admin/settings readiness check. It is read-only, no-mutation, sends no customer email, and returns provider/SMTP booleans, sender-domain SPF/DKIM/DMARC evidence status, relay diagnostics status, bounce/retry/webhook/suppression/alerting/monitoring gaps, blockers, warnings, `productionReady`, and redaction guarantees without raw SMTP host, username, password, API key, token, connection URL, authorization header, private DKIM key, webhook secret, or provider secret values.
