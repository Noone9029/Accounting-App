...
To isolate the late deployed accounting tail without running the full accounting smoke, use:
...
The tail phase independently creates its own smoke records and covers the ZATCA-safe no-network checks, AR/AP payments and allocations, refunds, credit notes, supplier payments/refunds, generated document downloads, uploaded attachments, representative audit log redaction, storage readiness, backup/restore readiness planning, dashboard/report endpoints, and PDF/CSV availability. The full smoke command remains available for complete local or long-ceiling validation.
