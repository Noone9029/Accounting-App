...
function printHelp() {
  console.log(
    [
      ...
      "  ZATCA_CSID_CUSTODY_PROVIDER                    Optional future provider mode; read only and never enables storage here.",
      "  ZATCA_CSID_CUSTODY_KMS_KEY_ID                  Optional future KMS key id; redacted if printed.",
      "  ZATCA_CSID_CUSTODY_SECRET_PREFIX               Optional future secret prefix; redacted if printed.",
      "  ZATCA_CSID_CUSTODY_REGION                      Optional future provider region; redacted if printed.",
      "  ZATCA_CSID_CUSTODY_ALLOW_BODY_STORAGE          Ignored in this phase; body storage remains blocked.",
      "",
      "Flags:",
      "  --egs-id <id>       EGS unit id. Defaults to the latest non-production EGS for the organization.",
      "  --mode plan|mock|real|custody  plan prints sanitized request plan; mock exercises local mock adapter only when env gate is true; real prints a blocker and performs no network; custody prints token/secret/certificate custody and disabled provider readiness only.",
      ...
      JSON.stringify(
        {
          ...
          custodyRecordRequired: "custodyRecordRequired" in plan ? plan.custodyRecordRequired : true,
          custodyRecordCount: "custodyRecordCount" in plan ? plan.custodyRecordCount : 0,
          latestCustodyRecord: "latestCustodyRecord" in plan ? plan.latestCustodyRecord : null,
          configuredProvider: "configuredProvider" in plan ? plan.configuredProvider : null,
          providerConfigPresent: "providerConfigPresent" in plan ? plan.providerConfigPresent : false,
          ...
          certificateStorageReady: "certificateStorageReady" in plan ? plan.certificateStorageReady : false,
          bodyPersistenceAllowed: "bodyPersistenceAllowed" in plan ? plan.bodyPersistenceAllowed : false,
          recommendedStorageMode: "recommendedStorageMode" in plan ? plan.recommendedStorageMode : null,
