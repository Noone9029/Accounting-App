...
export class LoopbackZatcaSandboxServer {
  ...
  private respondScenario(response: import("node:http").ServerResponse) {
    ...
    if (this.scenario === "CSID_DUPLICATE_JSON") { response.statusCode = 200; response.setHeader("content-type", "application/json"); return response.end('{"requestID":"synthetic-request","requestID":"duplicate","dispositionMessage":"ISSUED","binarySecurityToken":"' + Buffer.from("synthetic-certificate").toString("base64") + '","secret":"synthetic-secret"}'); }
