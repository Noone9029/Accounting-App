...
export class ChartOfAccountsService {
  ...
  async remove(organizationId: string, actorUserId: string, id: string) {
    ...
    if (lineCount > 0 || childCount > 0 || existing.isSystem) {
      throw new BadRequestException("Account can only be deleted when it is non-system, unused, and has no child accounts.");
    }
