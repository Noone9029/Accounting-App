...
export const PERMISSION_GROUPS: PermissionGroup[] = [
  ...
  },
  {
    ...
    permissions: [
      ...
      permission(PERMISSIONS.bankIntegrations.connectionManage, "Manage bank integration readiness", "Review Wio-shaped connection readiness and disabled-provider local mock boundaries."),
      permission(PERMISSIONS.bankIntegrations.feedRead, "Read bank integration feeds", "Review local/mock feed readiness without live bank-feed access."),
      permission(PERMISSIONS.bankIntegrations.beneficiaryManage, "Manage bank beneficiary mappings", "Create and review local beneficiary mapping metadata without bank credentials."),
      permission(PERMISSIONS.bankIntegrations.vendorPaymentCreate, "Create bank payment requests", "Create local vendor payment request metadata with provider release blocked by default."),
      permission(PERMISSIONS.bankIntegrations.vendorPaymentApprove, "Approve bank payment requests", "Approve local vendor payment request metadata without moving money."),
      permission(PERMISSIONS.bankIntegrations.vendorPaymentReconcile, "Reconcile bank payment requests", "Link bank payment request metadata to local feed or statement references."),
      ...
      permission(PERMISSIONS.bankIntegrations.connectionManage, "Manage bank connections", "Configure disabled-by-default bank integration readiness settings."),
      permission(PERMISSIONS.bankIntegrations.feedRead, "View bank feed readiness", "See bank feed readiness, provider status, and local diagnostic metadata."),
      permission(PERMISSIONS.bankIntegrations.beneficiaryManage, "Manage beneficiaries", "Review beneficiary readiness records without connecting real bank providers."),
      permission(PERMISSIONS.bankIntegrations.vendorPaymentCreate, "Create vendor payment readiness", "Create supplier payment readiness records without moving money."),
      permission(PERMISSIONS.bankIntegrations.vendorPaymentApprove, "Approve vendor payment readiness", "Approve supplier payment readiness records without provider execution."),
      permission(PERMISSIONS.bankIntegrations.vendorPaymentReconcile, "Reconcile vendor payment readiness", "Match supplier payment readiness records to local accounting state."),
  ...
  },
  {
    ...
    permissions: [
      ...
      permission(PERMISSIONS.payments.providerReadinessView, "View payment readiness", "See payment provider readiness, disabled-provider blockers, and beta payment-link status."),
      permission(PERMISSIONS.payments.paymentLinksCreate, "Create payment-link readiness records", "Create gated sales-invoice payment-link readiness records without initiating real provider payments by default."),
      permission(PERMISSIONS.payments.providerEventsManage, "Manage payment provider events", "Receive and review payment provider event records with signature and redaction controls."),
    ],
  },
  {
    id: "purchases",
  ...
  },
  {
    ...
    permissions: [
      ...
      permission(PERMISSIONS.documentInbox.view, "View document inbox", "See uploaded bill and receipt review queue rows."),
      permission(PERMISSIONS.documentInbox.upload, "Create document inbox items", "Create review inbox rows from existing immutable attachments."),
      permission(PERMISSIONS.documentInbox.review, "Review document inbox", "Run configured extraction and record accountant review decisions."),
      ...
      permission(PERMISSIONS.migrationToolkit.view, "View import/export toolkit", "See local CSV templates, preview jobs, validation issues, and migration limitations."),
      permission(PERMISSIONS.migrationToolkit.import, "Preview local imports", "Upload pasted local CSV content for row validation without accounting mutation."),
      permission(PERMISSIONS.migrationToolkit.commit, "Commit reviewed imports", "Commit validated local master-data imports after explicit review confirmation."),
      permission(PERMISSIONS.migrationToolkit.export, "Export migration CSVs", "Export tenant-scoped master data to CSV with formula-injection protection."),
