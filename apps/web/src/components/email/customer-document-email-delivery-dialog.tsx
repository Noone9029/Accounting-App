    ...
    <Dialog open={props.open} onOpenChange={(nextOpen) => { if (!props.submitting) props.onOpenChange(nextOpen); }}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        ...
        <form onSubmit={props.onSubmit} className="space-y-4">
          <LedgerFieldLabel htmlFor={`${idPrefix}-recipient`}><LedgerFieldText>Recipient email</LedgerFieldText><LedgerInput id={`${idPrefix}-recipient`} type="email" value={props.recipientEmail} onChange={(event) => props.onChange("recipientEmail", event.target.value)} disabled={props.submitting} required /><LedgerFieldHelp>Prefilled from the contact when available; you can override it for this delivery.</LedgerFieldHelp></LedgerFieldLabel>
...
export { CustomerDocumentEmailDeliveryDialog as DocumentEmailDeliveryDialog };
