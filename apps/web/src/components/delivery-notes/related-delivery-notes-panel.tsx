  ...
  return (
    <div className="rounded-md border border-slate-200 bg-white p-5 shadow-panel">
      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        ...
        <span className="self-start rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-700">Non-posting fulfillment</span>
      </div>

      <div className="mt-4">
        {loading ? <StatusMessage type="loading">Loading related delivery notes...</StatusMessage> : null}
        {!loading && deliveryNotes.length === 0 ? <StatusMessage type="empty">No delivery notes are linked to this {sourceKind} yet.</StatusMessage> : null}
              ...
              <tbody className="divide-y divide-slate-100">
                {deliveryNotes.map((deliveryNote) => (
                  <tr key={deliveryNote.id}>
                    ...
                    <td className="px-3 py-2">
                      <span className={`rounded-md px-2 py-1 text-xs font-medium ${deliveryNoteStatusBadgeClass(deliveryNote.status)}`}>
