...
export function PurchaseOrderForm({ initialOrder }: PurchaseOrderFormProps) {
  ...
  useEffect(() => {
    ...
    const querySupplierId = initialOrder ? "" : query.get("supplierId") ?? "";
    if (!initialOrder && querySupplierId) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((loadError: unknown) => {
        if (!cancelled) {
          setError(loadError instanceof Error ? loadError.message : tc("Unable to load purchase order setup data."));
        }
      })
      .finally(() => {
        if (!cancelled) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(submitError instanceof Error ? submitError.message : tc("Unable to save purchase order."));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (initialOrder && initialOrder.status !== "DRAFT") {
    return (
      <div className="space-y-4">
        <StatusMessage type="error">{tc("Only draft purchase orders can be edited.")}</StatusMessage>
        <Link
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      <div className="rounded-md border border-slate-200 bg-white p-5 shadow-panel">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <label className="block">
            <span className="text-sm font-medium text-slate-700">{tc("Order date")}</span>
            <input
              type="date"
              value={orderDate}
          ...
          <label className="block">
            <span className="text-sm font-medium text-slate-700">{tc("Expected delivery")}</span>
            <input
              type="date"
            ...
            {lines.map((line, index) => (
              <tr key={line.id}>
                <td className="px-4 py-3">
                  <select aria-label={`Item for purchase order line ${index + 1}`} value={line.itemId} onChange={(event) => selectItem(line.id, event.target.value)} className="w-full rounded-md border border-slate-300 px-2 py-1 text-sm outline-none focus:border-palm">
                ...
                <td className="px-4 py-3">
                  <input aria-label={`Description for purchase order line ${index + 1}`} value={line.description} onChange={(event) => updateLine(line.id, { description: event.target.value })} required className="w-full rounded-md border border-slate-300 px-2 py-1 text-sm outline-none focus:border-palm" />
                </td>
                <td className="px-4 py-3">
                  <select aria-label={`Purchase account for purchase order line ${index + 1}`} value={line.accountId} onChange={(event) => updateLine(line.id, { accountId: event.target.value })} className="w-full rounded-md border border-slate-300 px-2 py-1 text-sm outline-none focus:border-palm">
                    <option value="">{tc("No account yet")}</option>
                ...
                <td className="px-4 py-3">
                  <input aria-label={`Quantity for purchase order line ${index + 1}`} inputMode="decimal" value={line.quantity} onChange={(event) => updateLine(line.id, { quantity: event.target.value })} required className="w-24 rounded-md border border-slate-300 px-2 py-1 text-sm outline-none focus:border-palm" />
                </td>
                <td className="px-4 py-3">
                  <input aria-label={`Unit price for purchase order line ${index + 1}`} inputMode="decimal" value={line.unitPrice} onChange={(event) => updateLine(line.id, { unitPrice: event.target.value })} required className="w-28 rounded-md border border-slate-300 px-2 py-1 text-sm outline-none focus:border-palm" />
                </td>
                <td className="px-4 py-3">
                  <input aria-label={`Discount rate for purchase order line ${index + 1}`} inputMode="decimal" value={line.discountRate} onChange={(event) => updateLine(line.id, { discountRate: event.target.value })} className="w-24 rounded-md border border-slate-300 px-2 py-1 text-sm outline-none focus:border-palm" />
                </td>
                <td className="px-4 py-3">
                  <select aria-label={`Tax rate for purchase order line ${index + 1}`} value={line.taxRateId} onChange={(event) => updateLine(line.id, { taxRateId: event.target.value })} className="w-full rounded-md border border-slate-300 px-2 py-1 text-sm outline-none focus:border-palm">
      ...
      <div className="space-y-3">
        ...
        <StatusMessage type="info">{tc("Accounts are optional on the purchase order, but every line needs an account or item expense account before conversion to a bill.")}</StatusMessage>
        <StatusMessage type="info">{tc("No AP posting, inventory movement, payment sending, or tax-authority submission happens from this form.")}</StatusMessage>
      </div>
