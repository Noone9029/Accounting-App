  ...
  if (initialTemplate && initialTemplate.status !== "DRAFT") {
    return (
      <div className="space-y-4">
        <StatusMessage type="error">Only draft recurring invoice templates can be edited.</StatusMessage>
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div className="rounded-md border border-slate-200 bg-white p-5 shadow-panel">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <label className="block">
            <span className="text-sm font-medium text-slate-700">Template number</span>
            <input value={initialTemplate?.templateNumber ?? templateNumberPreview?.templateNumber ?? "From sequence"} readOnly aria-label="Template number" className="mt-1 w-full rounded-md border border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none" />
          ...
          <label className="block">
            <span className="text-sm font-medium text-slate-700">Payment terms days</span>
      ...
      <div className="space-y-3">
        {!organizationId ? <StatusMessage type="info">Log in and select an organization before creating recurring invoice templates.</StatusMessage> : null}
        ...
        {!loading && organizationId && postingRevenueAccounts.length === 0 ? (
          <StatusMessage type="empty">
            Add or activate a posting revenue account before saving recurring invoice lines.{" "}
      ...
      <div className="rounded-md border border-slate-200 bg-white p-5 shadow-panel">
        <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
          ...
          <span className="self-start rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-700">{frequencyLabel(frequency)}</span>
        </div>
        ...
        {lines.map((line, index) => {
          ...
          return (
            <div key={line.id} className="grid min-w-[1240px] grid-cols-[1fr_1.2fr_1.25fr_0.55fr_0.65fr_0.55fr_0.8fr_0.7fr_0.45fr] gap-3 border-b border-slate-100 px-4 py-3">
              <select aria-label={`Item for recurring invoice line ${index + 1}`} value={line.itemId} onChange={(event) => selectItem(line.id, event.target.value)} className="rounded-md border border-slate-300 px-2 py-2 text-sm">
              ...
              <input aria-label={`Description for recurring invoice line ${index + 1}`} value={line.description} onChange={(event) => updateLine(line.id, { description: event.target.value })} required className="rounded-md border border-slate-300 px-2 py-2 text-sm" />
              ...
              <input aria-label={`Quantity for recurring invoice line ${index + 1}`} inputMode="decimal" value={line.quantity} onChange={(event) => updateLine(line.id, { quantity: event.target.value })} className="rounded-md border border-slate-300 px-2 py-2 text-sm" />
              <input aria-label={`Price for recurring invoice line ${index + 1}`} inputMode="decimal" value={line.unitPrice} onChange={(event) => updateLine(line.id, { unitPrice: event.target.value })} className="rounded-md border border-slate-300 px-2 py-2 text-sm" />
              <input aria-label={`Discount rate for recurring invoice line ${index + 1}`} inputMode="decimal" value={line.discountRate} onChange={(event) => updateLine(line.id, { discountRate: event.target.value })} className="rounded-md border border-slate-300 px-2 py-2 text-sm" />
              <select aria-label={`Tax rate for recurring invoice line ${index + 1}`} value={taxMode === "NO_TAX" ? "" : line.taxRateId} onChange={(event) => updateLine(line.id, { taxRateId: event.target.value })} disabled={taxMode === "NO_TAX"} className="rounded-md border border-slate-300 px-2 py-2 text-sm disabled:bg-slate-50 disabled:text-slate-400">
...
function AccountPicker({
  ...
  return (
    <div className="space-y-1">
      <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} aria-label={`Search posting account for recurring invoice line ${lineNumber}`} placeholder="Search accounts" className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-xs outline-none focus:border-palm" />
      <select value={value} onChange={(event) => onChange(event.target.value)} required aria-label={`Posting account for recurring invoice line ${lineNumber}`} className="w-full rounded-md border border-slate-300 px-2 py-2 text-sm">
