...
describe("EditRecurringTransactionPage", () => {
  ...
  it("updates with optimistic template version evidence", async () => {
    ...
    await waitFor(() => expect(apiRequestMock).toHaveBeenCalledWith("/recurring-transactions/template-1", expect.objectContaining({ method: "PATCH", body: expect.objectContaining({ expectedVersion: 3, name: "Updated support" }) })));
    expect(pushMock).toHaveBeenCalledWith("/recurring-transactions/template-1");
  });
});

function templateFixture() {
  return { id: "template-1", templateCode: "REC-000001", transactionType: "SALES_INVOICE", name: "Monthly support", status: "DRAFT", timezone: "Asia/Dubai", frequency: "MONTHLY", interval: 1, startDate: "2026-07-01T00:00:00.000Z", nextRunAt: "2026-08-01T00:00:00.000Z", catchUpPolicy: "RUN_LATEST_ONLY", currencyCode: "AED", exchangeRatePolicy: "BASE_CURRENCY_ONLY", paymentTermsDays: 0, total: "100", templateVersion: 3, partyId: "customer-1", branchId: null, paidThroughAccountId: null, reference: null, notes: null, lines: [{ id: "line-1", accountId: "revenue-1", description: "Support", quantity: "1", unitPrice: "100", discountRate: "0", debit: "0", credit: "0", sortOrder: 0 }], runs: [] };
}
