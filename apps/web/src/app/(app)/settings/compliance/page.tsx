...
export default function ComplianceSettingsPage() {
  ...
  useEffect(() => {
    if (!organizationId || !edition.showsUaeEinvoicing) {
      return;
    }
    let cancelled = false;
    setLoading(true);
    ...
    Promise.all([getComplianceReadiness(), getOrganization(organizationId)])
      ...
      .catch((loadError: unknown) => {
        if (!cancelled) {
          setError(loadError instanceof Error ? loadError.message : tc("Unable to load compliance readiness."));
        }
      })
      .finally(() => {
        if (!cancelled) {
  ...
  }, [edition.showsUaeEinvoicing, organizationId]);
  ...
  async function saveOrganizationReadiness(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccess(tc("Updated UAE eInvoicing readiness fields."));
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : tc("Unable to update UAE readiness fields."));
    } finally {
      setSaving(false);
    }
  }
  ...
  if (!edition.showsUaeEinvoicing) {
    return (
      <section className="space-y-6">
        <header>
          <h1 className="text-2xl font-semibold text-ink">{tc("Compliance readiness")}</h1>
          <p className="mt-1 text-sm text-steel">{tc(edition.complianceDashboardNote)}</p>
        </header>
        <StatusMessage type="info">
          {edition.showsKsaZatca
            ? tc("KSA/ZATCA readiness is managed from ZATCA readiness settings.")
            : tc("Country-specific eInvoicing modules are disabled in the generic LedgerByte workspace.")}
        </StatusMessage>
      </section>
    );
  }
  ...
  return (
    <section className="space-y-6">
      <header>
        <h1 className="text-2xl font-semibold text-ink">{tc("Compliance readiness")}</h1>
        <p className="mt-1 text-sm text-steel">{tc("Track LedgerByte compliance-core status, UAE Peppol/PINT-AE data readiness, and disabled ASP connectivity.")}</p>
      </header>

      {!organizationId ? <StatusMessage type="info">{tc("Log in and select an organization to view compliance readiness.")}</StatusMessage> : null}
      {loading ? <StatusMessage type="loading">{tc("Loading compliance readiness...")}</StatusMessage> : null}
        ...
        <>
          <section className="rounded-md border border-sky-200 bg-sky-50 p-4 text-sm leading-6 text-sky-900">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="font-semibold text-sky-950">{tc("Controlled beta compliance foundation")}</h2>
                <p className="mt-1">{readiness.claim}</p>
                <p className="mt-1">{tc("No ASP, FTA, ZATCA, signing, clearance, reporting, or provider network call is enabled by default.")}</p>
              </div>
            </div>
          </section>

          <section className="grid gap-4 lg:grid-cols-3">
            <Metric label="Readiness" value={tc(complianceStatusLabel(readiness.uae.readiness.status))} />
          ...
          {organization ? (
            <section className="rounded-md border border-slate-200 bg-white p-5 shadow-panel">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <h2 className="text-base font-semibold text-ink">{tc("UAE eInvoicing readiness fields")}</h2>
                  <p className="mt-1 text-sm text-steel">{tc("Peppol/PINT-AE readiness and ASP connectivity preparation fields. These do not submit to an ASP or report to the FTA.")}</p>
                </div>
                <span className="rounded-md bg-slate-100 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-slate-700">{tc("Controlled beta")}</span>
              </div>
              <form onSubmit={saveOrganizationReadiness} className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
                ...
                <div className="md:col-span-3">
                  <button type="submit" disabled={!canUpdateOrganization || saving} className="rounded-md bg-palm px-4 py-2 text-sm font-semibold text-white hover:bg-teal-800 disabled:cursor-not-allowed disabled:bg-slate-400">
                    {saving ? tc("Saving...") : tc("Save UAE readiness fields")}
                  </button>
                  {!canUpdateOrganization ? <span className="ms-3 text-xs text-amber-700">{tc("Requires organization update permission.")}</span> : null}
                </div>
              </form>
            </section>
          ) : null}
          ...
          <section className="rounded-md border border-slate-200 bg-white p-5 shadow-panel">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-base font-semibold text-ink">{tc("UAE Peppol/PINT-AE checklist")}</h2>
              ...
              <span className="rounded-md bg-slate-100 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-slate-700">
                {readiness.noNetworkByDefault ? tc("No network") : tc("Network configured")}
              </span>
            </div>
          ...
          <section className="grid gap-4 lg:grid-cols-2">
            <section className="rounded-md border border-slate-200 bg-white p-5 shadow-panel">
              <h2 className="text-base font-semibold text-ink">{tc("UAE rollout dates")}</h2>
              <div className="mt-3 space-y-3">
                {readiness.uae.deadlines.map((deadline) => (
                  <div key={deadline.segment} className="rounded-md border border-slate-200 p-3 text-sm">
                    ...
                    <p className="mt-1 text-steel">{tc("ASP appointment by {appointAspBy}; implementation by {implementBy}.", { appointAspBy: deadline.appointAspBy, implementBy: deadline.implementBy })}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="rounded-md border border-slate-200 bg-white p-5 shadow-panel">
              <h2 className="text-base font-semibold text-ink">{tc("Official references")}</h2>
          ...
          <section className="rounded-md border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
            <h2 className="font-semibold text-amber-950">{tc("Claims to avoid")}</h2>
...
function Metric({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="rounded-md border border-slate-200 bg-white p-4 shadow-panel">
      <p className="text-xs font-semibold uppercase tracking-wide text-steel">{tc(label)}</p>
...
function ReadinessInput({ name, label, defaultValue, disabled }: { name: string; label: string; defaultValue?: string | null; disabled: boolean }) {
  ...
  return (
    <label className="block">
      <span className="text-xs font-medium uppercase tracking-wide text-steel">{tc(label)}</span>
      <input
        name={name}
...
function ReadinessCheckRow({ check }: { check: ComplianceReadinessCheck }) {
  ...
  return (
    <div className="flex gap-3 rounded-md border border-slate-200 p-3">
      ...
      <div>
        <p className="text-sm font-semibold text-ink">{tc(check.label)}</p>
