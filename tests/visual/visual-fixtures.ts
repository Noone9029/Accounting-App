...
const creditNote = {
  ...
  reversalJournalEntryId: null,
  customer,
  originalInvoice: { id: invoice.id, invoiceNumber: invoice.invoiceNumber, issueDate: invoice.issueDate, status: invoice.status, total: invoice.total, customerId: invoice.customerId },
  branch: null,
...
const customerRefund = {
  ...
  sourcePaymentId: customerPayment.id,
  sourceCreditNoteId: null,
  ...
  accountId: bankAccount.accountId,
  description: "Visual customer refund.",
  ...
  sourceCreditNote: null,
  journalEntry: { id: "journal-customer-refund-1", entryNumber: "JE-CREF-001", status: "POSTED" },
  voidReversalJournalEntry: null,
};

const collectionCase = {
  id: "collection-case-1",
  organizationId: org.id,
  ...
  promisedAmount: "325.0000",
  assignedToUserId: null,
  lastActivityAt: fixedVisualDate,
  nextActionAt: "2026-05-24T00:00:00.000Z",
  summary: "Customer promised payment after internal follow-up.",
  notes: "Visual collection case.",
  createdById: "user-1",
  updatedById: "user-1",
  createdAt: fixedVisualDate,
  updatedAt: fixedVisualDate,
  ...
  assignedTo: { id: "user-1", name: "Visual Tester", email: "visual@example.test" },
  createdBy: null,
  updatedBy: null,
  activities: [
    {
      ...
      organizationId: org.id,
      collectionCaseId: "collection-case-1",
      customerId: customer.id,
      salesInvoiceId: invoice.id,
      activityType: "PROMISE_TO_PAY",
      activityDate: fixedVisualDate,
      note: "Customer promised payment during internal follow-up.",
      nextFollowUpDate: "2026-05-24T00:00:00.000Z",
      promisedPaymentDate: "2026-05-25T00:00:00.000Z",
      promisedAmount: "325.0000",
      createdById: "user-1",
      createdAt: fixedVisualDate,
...
const collectionSummary = {
  totalOverdueAmount: invoice.balanceDue,
  overdueInvoiceCount: 1,
  openCollectionCaseCount: 1,
  casesDueToday: 0,
  casesOverdueForFollowUp: 1,
  promisedToPayTotal: collectionCase.promisedAmount,
  disputedTotal: "0.0000",
  topCustomersByOverdueAmount: [{ customerId: customer.id, customerName: customer.displayName, overdueAmount: invoice.balanceDue, overdueInvoiceCount: 1 }],
  agingBuckets: [{ bucket: "1_30", amount: invoice.balanceDue }],
...
const supplierRefund = {
  ...
  sourceDebitNoteId: debitNote.id,
  refundDate: "2026-05-21T00:00:00.000Z",
  currency: "SAR",
  status: "POSTED",
  amountRefunded: "115.0000",
  accountId: bankAccount.accountId,
  description: "Visual supplier refund.",
  ...
  journalEntry: { id: "journal-supplier-refund-1", entryNumber: "JE-SREF-001", status: "POSTED" },
  voidReversalJournalEntry: null,
};
...
function visualApiResponse(pathname: string, searchParams: URLSearchParams): MockResponse | null {
  ...
  if (pathname === "/credit-notes/credit-note-1") {
    return json(draftCreditNote);
  }
  if (pathname === "/customer-refunds") {
    return json([customerRefund]);
  }
  ...
  if (pathname === "/customer-refunds/customer-refund-1") {
    return json(customerRefund);
  }
  if (pathname === "/customer-refunds/customer-refund-1/pdf-data") {
    return json(customerRefundPdfData());
  }
  if (pathname === "/collections/summary") {
    return json(collectionSummary);
  }
  if (pathname === "/collections/next-number") {
    return json({ caseNumber: "COL-VIS-002", helperText: "Assigned from the collection case sequence when saved." });
  }
  if (pathname === "/collections/collection-case-1") {
  ...
  if (pathname === "/supplier-refunds/supplier-refund-1") {
    return json(supplierRefund);
  }
  if (pathname === "/supplier-refunds/supplier-refund-1/pdf-data") {
    return json(supplierRefundPdfData());
  }
  ...
  if (pathname === "/bank-accounts/bank-1/reconciliations") {
    return json(bankReconciliations());
  }
  if (pathname === "/bank-reconciliations/rec-1") {
    return json(bankReconciliations()[0]);
  }
  if (pathname === "/bank-reconciliations/rec-1/items") {
    return json(bankReconciliationItems());
  }
  if (pathname === "/bank-reconciliations/rec-1/review-events") {
    return json(bankReconciliationReviewEvents());
  }
  if (pathname === "/bank-reconciliations/rec-1/report-data") {
    return json(bankReconciliationReportData());
  }
  ...
  if (pathname === "/audit-logs") {
    return json(auditLogs(searchParams));
  }
...
function auditLogs(searchParams: URLSearchParams) {
  const entityType = searchParams.get("entityType") ?? "CustomerPayment";
  const entityId = searchParams.get("entityId") ?? "payment-1";
  return {
    data: [
      {
        id: `audit-${entityId}`,
        organizationId: org.id,
        actorUserId: "user-visual",
        actorUser: { id: "user-visual", name: "Visual Accountant", email: "accountant@example.test" },
        action: entityType === "CustomerPayment" ? "CUSTOMER_PAYMENT_CREATED" : "CUSTOMER_PAYMENT_UNAPPLIED_APPLIED",
        entityType,
        entityId,
        before: null,
        after: { paymentNumber: customerPayment.paymentNumber },
        metadata: {},
        ipAddress: null,
        userAgent: null,
        createdAt: fixedVisualDate,
  ...
  return [
    {
      ...
      bankAccountProfile: bankAccount,
      createdBy: { id: "user-1", name: "Visual User", email: "visual@example.test" },
      submittedBy: { id: "user-1", name: "Visual User", email: "visual@example.test" },
      approvedBy: { id: "user-1", name: "Visual User", email: "visual@example.test" },
      reopenedBy: null,
      closedBy: { id: "user-1", name: "Visual User", email: "visual@example.test" },
      voidedBy: null,
      _count: { items: 2 },
    },
  ];
...
function bankReconciliationItems() {
  return [
    {
      id: "rec-item-1",
      organizationId: org.id,
      reconciliationId: "rec-1",
      statementTransactionId: "statement-transaction-1",
      ...
      statementTransaction: {
        ...
        reference: "BNK-SETTLE-001",
        type: "CREDIT",
        amount: "750.0000",
        status: "MATCHED",
        matchedJournalLineId: "journal-line-1",
        matchedJournalEntryId: "journal-payment-1",
        matchType: "MANUAL",
        ...
        createdAt: fixedVisualDate,
        updatedAt: fixedVisualDate,
        matchedJournalEntry: { id: "journal-payment-1", entryNumber: "JE-PAY-001", entryDate: "2026-05-20T00:00:00.000Z", description: "Matched customer payment", reference: "BNK-SETTLE-001" },
    ...
    },
    {
      ...
      organizationId: org.id,
      reconciliationId: "rec-1",
      statementTransactionId: "statement-transaction-2",
      statusAtClose: "CATEGORIZED",
      amount: "500.0000",
      type: "DEBIT",
      createdAt: fixedVisualDate,
      statementTransaction: {
        id: "statement-transaction-2",
        organizationId: org.id,
        importId: "statement-import-1",
        bankAccountProfileId: bankAccount.id,
        ...
        matchType: null,
        categorizedAccountId: "bank-fees-account",
        createdJournalEntryId: "journal-fee-1",
        ignoredReason: null,
        createdAt: fixedVisualDate,
        updatedAt: fixedVisualDate,
        matchedJournalEntry: null,
        createdJournalEntry: { id: "journal-fee-1", entryNumber: "JE-FEE-001", entryDate: "2026-05-21T00:00:00.000Z", description: "Categorized bank charges", reference: "BNK-FEE-001" },
      },
    },
  ];
}

function bankReconciliationReviewEvents() {
  return [
    {
      id: "review-event-close",
      organizationId: org.id,
      reconciliationId: "rec-1",
      actorUserId: "user-1",
      action: "CLOSE",
    ...
    },
    {
      ...
      toStatus: "APPROVED",
      notes: "Approved with zero difference.",
...
function bankReconciliationReportData() {
  return {
    organization: org,
    currency: "SAR",
    reconciliation: {
    ...
    items: bankReconciliationItems().map((item) => ({
      id: item.id,
    ...
    summary: {
      ...
      creditRowsCount: 1,
      debitRowsCount: 1,
      creditRowsTotal: "750.0000",
      debitRowsTotal: "500.0000",
      exceptionRowsCount: 0,
    },
    linkedTreasurySummary: {
      depositBatches: { count: 1, matchedCount: 1, journalPostedCount: 1, operationalOnlyCount: 0, totalAmount: "750.0000" },
      cardSettlements: { count: 0, matchedCount: 0, journalPostedCount: 0, operationalOnlyCount: 0, totalAmount: "0.0000" },
      cheques: { count: 0, matchedCount: 0, journalPostedCount: 0, operationalOnlyCount: 0, totalAmount: "0.0000" },
    },
    accountingStatusSummary: {
      clearingConfigEnabled: true,
      configuredAccountCount: 2,
      journalPostedCount: 2,
    ...
    auditTimeline: [
      ...
      },
      {
        ...
        entityId: "statement-transaction-1",
        status: "MATCHED",
        actor: { id: "user-1", name: "Visual User", email: "visual@example.test" },
        amount: "750.0000",
        reference: "BNK-SETTLE-001",
...
function supplierRefundPdfData() {
  return {
    organization: org,
    supplier,
    refund: {
      id: supplierRefund.id,
      refundNumber: supplierRefund.refundNumber,
      refundDate: supplierRefund.refundDate,
      status: supplierRefund.status,
      currency: supplierRefund.currency,
      amountRefunded: supplierRefund.amountRefunded,
      description: supplierRefund.description,
    },
    source: {
      type: supplierRefund.sourceType,
      id: debitNote.id,
      number: debitNote.debitNoteNumber,
      date: debitNote.issueDate,
      status: debitNote.status,
      originalAmount: debitNote.total,
      remainingUnappliedAmount: debitNote.unappliedAmount,
    },
    receivedIntoAccount: bankAccount.account,
    journalEntry: supplierRefund.journalEntry,
    voidReversalJournalEntry: supplierRefund.voidReversalJournalEntry,
    generatedAt: fixedVisualDate,
  };
}

function customerRefundPdfData() {
  return {
    organization: org,
    customer,
    refund: {
      id: customerRefund.id,
      refundNumber: customerRefund.refundNumber,
      refundDate: customerRefund.refundDate,
      status: customerRefund.status,
      currency: customerRefund.currency,
      amountRefunded: customerRefund.amountRefunded,
      description: customerRefund.description,
    },
    source: {
      type: customerRefund.sourceType,
      id: customerPayment.id,
      number: customerPayment.paymentNumber,
      date: customerPayment.paymentDate,
      status: customerPayment.status,
      originalAmount: customerPayment.amountReceived,
      remainingUnappliedAmount: "25.0000",
    },
    paidFromAccount: bankAccount.account,
    journalEntry: customerRefund.journalEntry,
    voidReversalJournalEntry: customerRefund.voidReversalJournalEntry,
    generatedAt: fixedVisualDate,
  };
}
