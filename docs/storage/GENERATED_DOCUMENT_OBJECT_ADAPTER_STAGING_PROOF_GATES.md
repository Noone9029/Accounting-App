...
2026-07-20 ARC-04 update: a real `S3GeneratedDocumentStorageAdapter` now exists behind the existing storage boundary and was proven only against disposable loopback MinIO with synthetic data. Its selection is configuration-gated (`GENERATED_DOCUMENT_STORAGE_PROVIDER=s3` plus complete S3 settings); database storage remains the runtime default and signed URLs remain disabled. The complete local-only evidence and boundaries are in `docs/storage/ARC_04_LOCAL_S3_OBJECT_STORAGE_PROOF.md`. These staging approval gates remain `BLOCKED`; no hosted storage was touched.
