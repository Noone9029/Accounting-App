...
- UAE branch can now continue toward data-entry UX, invoice/credit-note validation panels, status timeline, and disabled/mock ASP connector.
- Remaining UAE blockers: editable forms, provider-specific ASP contract, EmaraTax/ASP onboarding evidence, legal retention re-verification, real non-production provider credentials, accountant/legal review, broad tests, and explicit approval before any real network call.
...
- Recommended next prompt: `UAE Peppol/PINT-AE data-entry UX and invoice validation panels`.
