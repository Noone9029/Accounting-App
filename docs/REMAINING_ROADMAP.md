...
- Use the new no-mutation hash comparison and dry-run reset plan to implement SDK/C14N11-backed hash persistence safely before replacing the local hash-chain behavior.
...
> Implement SDK/C14N11-backed ZATCA hash persistence behind explicit local-only flags, using the no-mutation hash comparison and dry-run reset plan as safety gates, without signing or network calls.
