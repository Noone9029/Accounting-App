...
- Use `LedgerPage` as the route wrapper and `LedgerPageHeader` for title, description, status badge, metadata, and actions. Header actions must wrap instead of creating document-level overflow on tablet widths.
...
- Purchase surfaces: `/purchases/bills`, `/purchases/bills/new`, `/purchases/bills/[id]`, `/purchases/bills/[id]/edit`, `/purchases/debit-notes`, `/purchases/debit-notes/new`, `/purchases/debit-notes/[id]`, and `/purchases/debit-notes/[id]/edit`.
