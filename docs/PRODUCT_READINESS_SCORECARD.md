...
| ZATCA | 38 | Profile, EGS, CSR groundwork, mock CSID, local XML/QR/hash, SDK readiness docs, disabled-by-default local SDK validation endpoints, official fixture registry/results doc, official sample validation pass under Java 11, local standard fixture SDK global pass, simplified fixture XSD/EN/PIH pass, API-generated standard XML local SDK validation, no-mutation SDK/app hash comparison, dry-run hash-chain reset plan, explicit fresh-EGS SDK hash mode, SDK-hash metadata snapshot, blocked real network behavior. | Signing, Phase 2 QR, CSID, clearance, reporting, PDF/A-3, generated XML address/identifier warnings, KMS key custody, and repeatable Java/SDK CI execution remain missing. Existing local hash chains are not migrated. | Validate SDK hash mode on fresh non-production EGS units, then design signing/certificate work. |
