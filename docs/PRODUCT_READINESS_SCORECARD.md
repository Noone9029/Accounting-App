...
| Banking/reconciliation | 73 | Bank profiles, transfers, opening balances, manual CSV/JSON/text plus limited OFX/CAMT/MT940/XLSX statement upload or paste preview, downloadable canonical CSV statement template UX, service-level duplicate/idempotency/reconciliation-overlap import safety, parser QA for common sanitized variants and safe errors, deterministic non-mutating match suggestions, deterministic bank-rule suggestions with explicit apply/audit behavior, inline statement transaction review workspace, matching, categorization, reconciliation approval/close/void, reports, design-only raw-file archive policy, visible banking/reconciliation drill-down guidance, and closed DEV-09 local evidence for synthetic fixtures, parser/preview checks, match/categorize/ignore, and reconciliation close/void. | No live feeds, auto-match execution, silent auto-rule application, DB-level unique statement fingerprint/index, certified bank-specific parser coverage, deposits, cheques, card settlements, transfer fees, FX handling, production raw-file archive workflow, broad E2E/smoke/full-test coverage, hosted/beta/customer-data behavior, or accountant sign-off. | Build bank deposit batches next; keep DB-level fingerprint/index as future hardening if concurrency risk requires it. |
