...
| Banking/reconciliation | 74 | Bank profiles, transfers, opening balances, manual CSV/JSON/text plus limited OFX/CAMT/MT940/XLSX statement upload or paste preview, downloadable canonical CSV statement template UX, service-level duplicate/idempotency/reconciliation-overlap import safety, parser QA for common sanitized variants and safe errors, deterministic non-mutating match suggestions, deterministic bank-rule suggestions with explicit apply/audit behavior, operational bank deposit batches with explicit statement-credit matching, inline statement transaction review workspace, matching, categorization, reconciliation approval/close/void, reports, design-only raw-file archive policy, visible banking/reconciliation drill-down guidance, and closed DEV-09 local evidence for synthetic fixtures, parser/preview checks, match/categorize/ignore, and reconciliation close/void. | No live feeds, auto-match execution, silent auto-rule application, DB-level unique statement fingerprint/index, certified bank-specific parser coverage, journal-backed undeposited-funds clearing for deposit batches, full cheques, card settlements, transfer fees, FX handling, production raw-file archive workflow, broad E2E/smoke/full-test coverage, hosted/beta/customer-data behavior, or accountant sign-off. | Build credit/prepaid card settlement flows next; keep bank-deposit journal clearing and DB-level fingerprint/index as future hardening once their models are explicitly designed. |
