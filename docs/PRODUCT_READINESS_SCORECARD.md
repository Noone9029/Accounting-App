...
| Email/communications | 60 | Mock email outbox default, invites, password reset, redacted readiness API/UI, disabled-by-default diagnostics and retry processor, metadata-only SPF/DKIM/DMARC evidence capture, opt-in SMTP adapter, test-send, durable retry metadata, metadata-only provider events, disabled-by-default signed webhook plan/test verifier, suppression metadata, DB-backed rate limits. | No scheduled retry worker, provider-specific production webhook adapter, live DNS/provider validation, executed relay validation, monitoring/alert thresholds, polished templates, invoice/statement sending, or MFA/session invalidation. | Add scheduled retry worker and monitoring dashboard evidence after provider-specific webhook review. |
...
2. Non-production SMTP relay execution, provider-specific signed webhooks, scheduled email retry worker, monitoring/alert thresholds, and live domain-auth validation around the opt-in SMTP adapter.
