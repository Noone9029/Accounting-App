...
| Email/communications | 58 | Mock email outbox default, invites, password reset, redacted readiness API/UI, disabled-by-default diagnostics and retry processor, metadata-only SPF/DKIM/DMARC evidence capture, opt-in SMTP adapter, test-send, durable retry metadata, mock provider event/bounce capture, DB-backed rate limits. | No scheduled retry worker, signed production webhooks, suppression list, live DNS/provider validation, executed relay validation, monitoring, polished templates, invoice/statement sending, or MFA/session invalidation. | Run an allowlisted non-production relay diagnostic and add signed webhook/monitoring controls. |
...
2. Non-production SMTP relay execution, signed provider webhooks/suppression handling, scheduled email retry worker, monitoring, and live domain-auth validation around the opt-in SMTP adapter.
