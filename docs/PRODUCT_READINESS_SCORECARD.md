...
| UX/product polish | 68 | Broad route coverage, dashboard KPI cards, lightweight charts/drill-downs, settings pages, panels, helper tests, permission-aware nav, guided first-workflow setup, invoice/payment success guidance, AR/AP ledger/report drill-down polish, and banking/reconciliation drill-down polish. | List filters, bulk actions, customizable dashboards, broader empty/error states, visual consistency, and accountant-reviewed report wording. | Continue route QA on inventory workflows without changing posting behavior. |
