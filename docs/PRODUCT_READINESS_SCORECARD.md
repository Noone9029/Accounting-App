...
| Purchases/AP | 80 | Purchase orders, bills, clearing mode, supplier payments, debit notes, refunds, cash expenses, AP ledgers/statements, PDFs, and guided AP drill-down panels. | Partial billing, multi-PO matching, remittance email, OCR/import, supplier statement PDF parity. | Harden purchase matching and supplier statement PDF parity. |
...
| Documents/attachments | 70 | Generated PDFs, archive, document action guidance, clearer template/settings copy, archive status labels, attachment upload/list/download/soft-delete, linked panels, storage readiness/migration dry run, feature-flagged S3-compatible storage for new uploaded attachments, backup evidence planning, and local restore-count evidence for database-backed records. | DB/base64 remains default, no migration executor, no generated-document S3 path, no virus scanning, no OCR, no retention policy, no real object-storage restore proof, no supplier statement PDF parity. | Validate S3 mode with a non-production bucket, implement migration executor, and capture object-storage restore evidence. |
