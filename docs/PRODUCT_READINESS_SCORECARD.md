...
| UX/product polish | 66 | Broad route coverage, dashboard KPI cards, lightweight charts/drill-downs, settings pages, panels, helper tests, permission-aware nav, guided first-workflow setup, invoice/payment success guidance, and AR/AP ledger/report drill-down polish. | List filters, bulk actions, customizable dashboards, broader empty/error states, visual consistency, and accountant-reviewed report wording. | Continue route QA on banking/reconciliation or inventory workflows without changing posting behavior. |
