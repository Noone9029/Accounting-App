...
| ZATCA | 41 | Profile, EGS, CSR groundwork, mock CSID, local XML/QR/hash, SDK readiness docs, disabled-by-default local SDK validation endpoints, repeatable local/no-network SDK validation wrapper, metadata-only evidence format, SDK fixture registry, generated standard invoice and credit-note fixture SDK pass under Java 11.0.26, SDK CI readiness guard with metadata-only blocked status, official fixture registry/results doc, official sample validation pass under Java 11, local standard fixture SDK global pass, simplified fixture XSD/EN/PIH pass, API-generated standard XML local SDK validation, no-mutation SDK/app hash comparison, dry-run hash-chain reset plan, explicit fresh-EGS SDK hash mode, SDK-hash metadata snapshot, invoice-specific PIH validation config, blocked real network behavior, and documented preparation gates for environment separation, key custody decision drafting, invoice eligibility, audit evidence, sandbox onboarding, and SDK validation readiness. | Signing, Phase 2 QR, production CSID, clearance, reporting, PDF/A-3, implemented KMS/key custody, error/retry queue, approved SDK reference/acquisition policy for CI, Java 11-14 CI runtime, metadata-only artifact retention approval, and repeatable Java/SDK CI execution remain missing. Existing local hash chains are not migrated. | Resolve SDK reference/acquisition and artifact retention policy, then wire a Java 11-14 no-network SDK CI runner after guard status can pass. |
