...
| Email/communications | 56 | Mock email outbox default, invites, password reset, redacted readiness API/UI, disabled-by-default diagnostics, metadata-only SPF/DKIM/DMARC evidence capture, opt-in SMTP adapter, test-send, DB-backed rate limits. | No queue/retries, bounces/webhooks, live DNS/provider validation, executed relay validation, polished templates, invoice/statement sending, or MFA/session invalidation. | Run an allowlisted non-production relay diagnostic and add deliverability/queue controls. |
...
2. Non-production SMTP relay execution, email queue/retry, provider webhooks, and live domain-auth validation around the opt-in SMTP adapter.
