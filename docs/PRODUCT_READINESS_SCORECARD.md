...
| Documents/attachments | 66 | Generated PDFs, archive, attachment upload/list/download/soft-delete, linked panels, storage readiness/migration dry run, feature-flagged S3-compatible storage for new uploaded attachments, and backup evidence planning. | DB/base64 remains default, no migration executor, no generated-document S3 path, no virus scanning, no OCR, no retention policy, no real restore drill. | Validate S3 mode with a non-production bucket, implement migration executor, and capture restore drill evidence. |
...
| Storage/scalability | 52 | Storage config/readiness, feature-flagged S3-compatible attachment upload/download, migration plan counts, database default works locally, and backup/restore evidence readiness is modeled. | No DB-to-S3 migration executor, signed URLs, object lifecycle, generated-document S3 path, scanning, executed backup policy, restore drill, or real-bucket validation evidence. | Test S3 mode against a non-production bucket and execute a restore-drill-backed migration rollback plan. |
...
| Deployment readiness | 50 | Vercel/Supabase docs, API health/root/readiness, deployed E2E runbook/workflow, CI DB checklist, Supabase security review, and backup/restore readiness plan/evidence endpoints. | No production IaC, executed backup/PITR proof, restore drill, monitoring, RLS/private-network decision, environment promotion policy. | Execute non-production restore drill evidence and add monitoring/incident runbooks. |
...
| Production operations | 30 | Readiness docs, manual smoke/E2E workflows, disabled email worker planning, and backup/restore evidence planning. | No incident response, observability, real background jobs, data retention executors, support tools, billing, SLAs, or executed restore drills. | Define operations baseline: monitoring, alerts, restore drills, runbooks, and support evidence. |
...
3. Execute non-production backup/restore drills and capture sanitized evidence for Supabase/Postgres, object storage, documents, attachments, RPO/RTO, and restore verification.
