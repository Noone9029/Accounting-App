...
| Purchases/AP | 80 | Purchase orders, bills, clearing mode, supplier payments, debit notes, refunds, cash expenses, AP ledgers/statements, PDFs, supplier statement PDF export, and guided AP drill-down panels. | Partial billing, multi-PO matching, remittance email, OCR/import, supplier statement archive tracking. | Harden purchase matching and supplier statement archive tracking. |
| Banking/reconciliation | 68 | Bank profiles, transfers, opening balances, manual CSV/JSON/text statement upload or paste preview, matching, categorization, reconciliation approval/close/void, reports, and visible banking/reconciliation drill-down guidance. | No live feeds, auto-match, OFX/CAMT/MT940 parser, transfer fees, FX handling, production raw-file archive workflow. | Add OFX/CAMT/MT940 samples and approval queue polish without live bank feeds. |
...
| Documents/attachments | 70 | Generated PDFs, archive, document action guidance, clearer template/settings copy, archive status labels, supplier statement PDF export, attachment upload/list/download/soft-delete, linked panels, storage readiness/migration dry run, feature-flagged S3-compatible storage for new uploaded attachments, backup evidence planning, and local restore-count evidence for database-backed records. | DB/base64 remains default, no migration executor, no generated-document S3 path, no virus scanning, no OCR, no retention policy, no real object-storage restore proof, no supplier statement archive type. | Validate S3 mode with a non-production bucket, implement migration executor, add reviewed supplier statement archive tracking if required, and capture object-storage restore evidence. |
...
| UX/product polish | 72 | Broad route coverage, dashboard KPI cards, lightweight charts/drill-downs, settings pages, panels, helper tests, permission-aware nav, guided first-workflow setup, invoice/payment success guidance, AR/AP ledger/report drill-down polish, banking/reconciliation drill-down polish, inventory drill-down polish, and document/PDF/template UX polish. | List filters, bulk actions, customizable dashboards, broader empty/error states, visual consistency, and accountant-reviewed report/document wording. | Add visual regression coverage and accountant wording review on high-traffic workflows. |
...
- Archive rows were created for invoice, customer payment receipt, credit note, purchase bill, supplier payment receipt, purchase debit note, and customer statement downloads. Supplier statement PDF download worked, but archive tracking remains a known limitation until a `SUPPLIER_STATEMENT` generated-document type can be added through a reviewed schema migration.
