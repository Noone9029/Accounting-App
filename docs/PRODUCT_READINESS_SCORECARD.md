...
| Deployment readiness | 54 | Vercel/Supabase docs, API health/root/readiness, deployed E2E runbook/workflow, CI DB checklist, Supabase security review, backup/restore readiness endpoints, and one local non-production Postgres restore drill. | No production IaC, hosted Supabase backup/PITR proof, real object-storage restore proof, monitoring, RLS/private-network decision, environment promotion policy. | Verify hosted Supabase PITR/object storage and add monitoring/incident runbooks. |
...
3. Verify hosted Supabase backup/PITR and real object-storage backup/restore in non-production, then capture sanitized evidence for PITR, object storage, RPO/RTO, and restore verification.
