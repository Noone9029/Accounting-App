...
| UAE eInvoicing readiness | 51 | Neutral compliance core, UAE readiness fields, editable organization and contact/customer/supplier data-entry UX, `0235{TIN}` participant ID helper, local readiness XML fixtures, official local PINT-AE serializer/rule-pack foundation with official identifiers, local invoice/credit-note readiness panels, explicit local validation actions, disabled/default ASP adapter, explicit mock-only adapter, future-provider placeholders, redacted config summary, metadata-only XML/evidence archive records, provider-selection/sandbox-contract planning docs, and provider outreach execution pack. | No selected provider, no provider-specific API contract, no provider response evidence, no sandbox docs received from a provider, unknown official commercial-invoice/predefined-endpoint/transaction-flag values still source-required TODOs, no EmaraTax automation, no provider credentials, no real FTA confirmation, no production retention guarantee, no legal/accountant review, no broad E2E/smoke/full-test coverage, and no production compliance claim. | Review official-code TODOs against current OpenPeppol/UAE docs, then collect provider sandbox payload evidence before any real network or provider submission work. |
...
- Unknown official commercial-invoice type-code mapping, predefined endpoint scenario values, and transaction flag mappings remain TODOs and are intentionally not guessed.
- Provider-specific payload contracts remain blocked on actual provider sandbox docs.
- ZATCA remains parked and blocked by default; no ZATCA production behavior, real network call, OTP, CSID, signing, clearance/reporting, or PDF-A3 behavior was enabled.
- Next priority is `UAE PINT-AE official-code TODO review`.
