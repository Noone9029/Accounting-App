...
| `BankAccountProfile` | Cash/bank metadata wrapper for a posting asset account. | `accountId`, `type`, `status`, `displayName`, masked account/IBAN, currency, opening balance metadata, `openingBalanceJournalEntryId`, `openingBalancePostedAt`. | Organization, one linked Account, opening-balance journal, incoming/outgoing transfers, statement imports/transactions. | Profile metadata itself does not post; optional opening-balance action posts one guarded journal and ledger balance is derived from posted journal lines. | `BankAccountStatus`. | No live feed, transfer fees, or FX transfer handling. |
...
| `BankStatementImport` | Local bank statement import batch. | `bankAccountProfileId`, `filename`, `sourceType`, `status`, statement dates, opening/closing statement balances, `rowCount`, `importedAt`. | Organization, BankAccountProfile, importedBy User, statement transactions. | Import itself creates no journals. | `BankStatementImportStatus`. | JSON/CSV-row MVP only; no file attachment or reconciliation close process. |
| `BankStatementTransaction` | Imported statement row for manual reconciliation. | `transactionDate`, `description`, `reference`, `type`, `amount`, `status`, matched/categorized journal links, `rawData`. | Organization, import batch, BankAccountProfile, optional matched JournalLine/JournalEntry, optional categorized Account, optional created JournalEntry. | Manual match creates no journal; categorization creates a posted balanced journal. | `BankStatementTransactionStatus`, `BankStatementTransactionType`, optional `BankStatementMatchType`. | No automatic/ML matching, bank feeds, or statement lock/approval workflow. |
