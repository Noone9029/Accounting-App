...
| `NumberSequenceScope` | `JOURNAL_ENTRY`, `INVOICE`, `PURCHASE_ORDER`, `BILL`, `PAYMENT`, `CUSTOMER_REFUND`, `CREDIT_NOTE`, `DEBIT_NOTE`, `PURCHASE_DEBIT_NOTE`, `SUPPLIER_REFUND`, `CASH_EXPENSE`, `BANK_TRANSFER`, `BANK_RECONCILIATION`, `CONTACT`, `INVENTORY_ADJUSTMENT`, `WAREHOUSE_TRANSFER` | Tenant numbering scopes. |
...
| `Item` | Product/service catalog. | `type`, `status`, `sellingPrice`, `purchaseCost`, account/tax defaults, `inventoryTracking`, optional `reorderPoint`, optional `reorderQuantity`. | Invoice, credit note, purchase order, purchase bill lines, stock movements, inventory adjustments, warehouse transfers. | Defaults accounts/taxes; inventory movements and reorder fields are operational only and do not post stock accounting yet. | `ItemStatus`. | No automatic sales/purchase stock movements, COGS, or valuation accounting. |
