...
| `BankReconciliation` | Formal bank account reconciliation record. | `reconciliationNumber`, `bankAccountProfileId`, `periodStart`, `periodEnd`, statement opening/closing balances, `ledgerClosingBalance`, `difference`, actor ids, close/void timestamps. | Organization, BankAccountProfile, created/closed/voided users, reconciliation items. | Creates no journals; closing snapshots statement rows and locks the period from statement row changes. | `BankReconciliationStatus`. | No report PDF, reviewer approval workflow, or close package export. |
