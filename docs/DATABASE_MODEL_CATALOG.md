...
| `StockMovementType` | `OPENING_BALANCE`, `ADJUSTMENT_IN`, `ADJUSTMENT_OUT`, `TRANSFER_IN`, `TRANSFER_OUT`, `PURCHASE_RECEIPT_PLACEHOLDER`, `SALES_ISSUE_PLACEHOLDER` | Operational stock ledger movement classification; MVP creation allows opening balances and manual adjustments only. |
...
| `NumberSequenceScope` | `JOURNAL_ENTRY`, `INVOICE`, `PURCHASE_ORDER`, `BILL`, `PAYMENT`, `CUSTOMER_REFUND`, `CREDIT_NOTE`, `DEBIT_NOTE`, `PURCHASE_DEBIT_NOTE`, `SUPPLIER_REFUND`, `CASH_EXPENSE`, `BANK_TRANSFER`, `BANK_RECONCILIATION`, `CONTACT` | Tenant numbering scopes. |
...
| `Warehouse` | Operational inventory location. | `code`, `name`, `status`, address fields, `countryCode`, `isDefault`. | Organization, StockMovement. | No direct accounting impact; active warehouses can receive manual stock movements. | `WarehouseStatus`. | No bin/location hierarchy, transfer workflow, or warehouse accounting. |
| `StockMovement` | Operational stock ledger row. | `itemId`, `warehouseId`, `movementDate`, `type`, positive `quantity`, optional `unitCost`, optional `totalCost`, reference fields, creator. | Organization, Item, Warehouse, optional createdBy User. | No journal entry; quantity ledger only. | Direction is derived from `StockMovementType`. | No COGS, valuation accounting, automatic receipts/issues, or transfer workflow yet. |
...
| `Item` | Product/service catalog. | `type`, `status`, `sellingPrice`, `purchaseCost`, account/tax defaults, `inventoryTracking`. | Invoice, credit note, purchase order, purchase bill lines, stock movements. | Defaults accounts/taxes; manual inventory movement is operational only and does not post stock accounting yet. | `ItemStatus`. | No automatic sales/purchase stock movements, COGS, or valuation accounting. |
