...
Frontend redesign loop engineering update: LedgerByte now has a full route-family redesign checklist and the first post-merge loop has started on Sales list surfaces. `/sales/quotes` and `/sales/invoices` list views use shared LedgerByte page, summary, filter, table, date, money, badge, action, and empty-state primitives. This preserves existing permissions and product truth: quotes remain non-posting, invoice finalization remains explicit, and no backend, provider, storage, signed URL, compliance, hosted, payment, email, ledger math, report math, VAT math, inventory valuation, seed/reset/delete, or shutdown behavior changed.
