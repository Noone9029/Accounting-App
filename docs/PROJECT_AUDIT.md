...
1. ZATCA is not production compliant; LedgerByte simplified XML still fails signing/QR/certificate checks, API-generated XML still has address warnings, fresh-EGS SDK hash persistence now works locally but generated invoice 2 still fails `KSA-13` PIH validation, and real onboarding, signing, and API submission are missing.
...
8. Resolve the fresh-EGS SDK-mode generated XML gaps, starting with invoice 2 `KSA-13` and buyer-address warnings, then advance signing/certificate, Phase 2 QR, CSID, clearance/reporting, and PDF/A-3.
