...
- Role and organization member management UI/API with custom role creation, permission matrices, role/status changes, active-provider email invites, invite acceptance, password reset, email provider readiness/diagnostics/sender-domain evidence/test-send, token request rate limits, expired-token cleanup, email outbox inspection, audit log review UI, filtered audit CSV export, retention dry-run controls, and guarded number-sequence settings.
...
- Email invitation, invited-user onboarding, and password reset exist with mock/local default delivery, an opt-in SMTP adapter, redacted provider readiness, disabled-by-default diagnostics, metadata-only SPF/DKIM/DMARC evidence, test-send, DB-backed rate limits, and expired-token cleanup; no background queue/retry, provider webhooks, live DNS/provider validation, executed relay validation, MFA, or advanced session management exists.
...
2. Invite/onboarding/password reset default to mock/local delivery with provider readiness, disabled-by-default diagnostics, sender-domain evidence, test-send, opt-in SMTP, and DB-backed request rate limits; executed relay validation, MFA, queue/retry, webhooks, and advanced session management are still missing.
...
2. Run SMTP diagnostics against a non-production relay using allowlisted diagnostics, add live DKIM/SPF/domain-authentication checks, MFA planning, scheduled audit export, audit alerting for role/member changes, and a reviewed number-sequence reset/skip workflow.
