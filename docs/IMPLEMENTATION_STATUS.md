...
| Production deployment | GROUNDWORK_ONLY | `infra`, `docs/DEPLOYMENT_VERCEL_SUPABASE.md`, `docs/deployment/*`, `docs/BACKUP_AND_RESTORE_READINESS_PLAN.md`, `.github/workflows/deployed-e2e.yml` | Local compose, Vercel/Supabase deployment checklist, API root/health/readiness docs, CI DB readiness checklist, Supabase security review, deployed E2E runbook/workflow, and metadata-only backup/restore readiness planning. | Production IaC, real backup validation, restore drills, monitoring, RLS/private-network decision, deployment approval gates, and operations runbooks are not complete. | Execute non-production backup/restore drill evidence, then create production readiness plan covering monitoring, secrets, storage, email, and security controls. |
| Cloud storage | PARTIAL | `apps/api/src/storage`, `apps/api/src/attachments`, `/settings/storage`, `docs/storage/*`, `apps/api/src/system` | Provider env config, database default readiness, redacted S3 config checks, feature-flagged S3-compatible upload/download storage for new uploaded attachments, dry-run migration counts for attachments/generated documents, and backup/object-storage evidence readiness on the storage settings page. | Database remains default; generated documents are still DB-backed; no migration executor, signed URL policy, object lifecycle/deletion policy, virus scanning, real-bucket production validation, or actual restore drill exists. | Test S3 mode with a non-production bucket, then add generated-document storage alignment, a resumable migration executor, and restore-drill evidence. |
...
- Full production operations readiness remains false until a real non-production restore drill, hosted backup/PITR proof, object-storage backup validation, RPO/RTO business review, monitoring, and incident runbooks are completed.
