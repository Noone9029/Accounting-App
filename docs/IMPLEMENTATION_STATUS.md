...
| Email outbox and provider routing | GROUNDWORK_ONLY | `EmailOutbox`, `EmailSenderDomainEvidence`, `EmailProviderEvent`, `EmailSuppression`, `AuthToken`, `AuthTokenRateLimitEvent`, `apps/api/src/email`, `/settings/email-outbox` | Mock remains default; invite/password reset/test-send records are stored in outbox; `GET /email/readiness` reports provider/SMTP production readiness, sender-domain SPF/DKIM/DMARC evidence status, relay diagnostics, retry counts/processor flag, webhook verification booleans, suppression counts, and bounce/provider-event/alerting/monitoring blockers without secrets; `POST /email/diagnostics` and default `POST /email/retry-process` return no-send/no-mutation skipped plans; SMTP adapter can be enabled by env and records `SENT_PROVIDER`; invite/reset delivery is DB-rate-limited; provider events and suppressions capture metadata-only bounce/complaint evidence. | No scheduled retry worker, provider-specific production webhook adapter, live DNS/provider validation, real relay execution evidence, monitoring dashboard/alerts, or paid/provider-specific API adapter. | Add a scheduled retry worker and monitoring/alert threshold evidence before real customer email use. |
...
| Email sending | GROUNDWORK_ONLY | `apps/api/src/email`, `EmailOutbox`, `EmailSenderDomainEvidence`, `EmailProviderEvent`, `EmailSuppression`, `AuthTokenRateLimitEvent`, `/settings/email-outbox`, `docs/email/*` | Mock/local organization invite and password reset email records are stored and inspectable by default; readiness endpoint and UI show production SMTP readiness, metadata-only SPF/DKIM/DMARC evidence, relay diagnostics status, retry processor/counts, provider event status, signed-webhook plan, suppression count, and no-customer-email safeguards without secrets; disabled-by-default diagnostics and retry processing return skipped/no-send/no-mutation status; webhook verification is disabled by default; `POST /email/test-send` still exercises the active provider unless a matched active suppression blocks it; DB-backed invite/reset rate limits and manual expired-token cleanup exist; SMTP can send only when explicitly configured. | No invoice/statement send, scheduled retry worker, provider-specific production webhook adapter, live DNS/provider validation, real relay execution evidence, or monitoring dashboard/alerts. | Add scheduled retry execution and monitoring/alert threshold evidence after provider-specific webhook contract review. |
