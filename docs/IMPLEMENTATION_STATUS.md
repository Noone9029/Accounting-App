...
| Email outbox and provider routing | GROUNDWORK_ONLY | `EmailOutbox`, `EmailSenderDomainEvidence`, `AuthToken`, `AuthTokenRateLimitEvent`, `apps/api/src/email`, `/settings/email-outbox` | Mock remains default; invite/password reset/test-send records are stored in outbox; `GET /email/readiness` reports provider/SMTP production readiness, sender-domain SPF/DKIM/DMARC evidence status, relay diagnostics, and bounce/retry/monitoring blockers without secrets; `POST /email/diagnostics` is disabled by default and returns no-send/no-mutation diagnostics; SMTP adapter can be enabled by env and records `SENT_PROVIDER`; invite/reset delivery is DB-rate-limited. | No background queue/retry worker, provider webhooks, bounce handling, live DNS/provider validation, real relay execution evidence, or paid/provider-specific API adapter. | Run an allowlisted non-production relay diagnostic, then add queue/retry and deliverability controls before real customer use. |
...
| Email sending | GROUNDWORK_ONLY | `apps/api/src/email`, `EmailOutbox`, `EmailSenderDomainEvidence`, `AuthTokenRateLimitEvent`, `/settings/email-outbox`, `docs/email/*` | Mock/local organization invite and password reset email records are stored and inspectable by default; readiness endpoint and UI show production SMTP readiness, metadata-only SPF/DKIM/DMARC evidence, relay diagnostics status, and no-customer-email safeguards without secrets; disabled-by-default diagnostics returns skipped/no-send/no-mutation status; `POST /email/test-send` still exercises the active provider; DB-backed invite/reset rate limits and manual expired-token cleanup exist; SMTP can send only when explicitly configured. | No invoice/statement send, retries, bounces, webhooks, live DNS/provider validation, real relay execution evidence, or background queue. | Run a non-production relay diagnostic with an allowlisted sandbox recipient, then add queue/retry and provider event handling. |
