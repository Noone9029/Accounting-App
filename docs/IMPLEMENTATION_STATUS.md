...
| Subscription billing | LOCAL_FOUNDATION_PROVEN | `apps/api/src/billing`, `apps/web/src/app/(app)/settings/billing`, `apps/web/src/app/(app)/plans`, `docs/billing/PAID_SAAS_LOCAL_PROOF.md` | Plan/version schema, entitlement engine, lifecycle states, fake-provider webhook boundary, tenant status UI/API, and disposable local proof exist. | Provider execution, live collection, public pricing, production enforcement, legal/tax approval, ZATCA/UAE sellability, and production launch remain disabled/not approved. | Review the local proof before any separately approved Stripe test-mode work. |
