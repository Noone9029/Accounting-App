...
| Stock movements | GROUNDWORK_ONLY | `StockMovement`, `apps/api/src/stock-movements`, web `/inventory/stock-movements` | Direct opening balance creation, generated adjustment/transfer movement rows, positive-quantity ledger, duplicate opening balance rejection, and negative stock prevention. | No purchase receipt, sales issue, landed cost, or accounting posting. | Add receipt/issue hooks after valuation policy. |
