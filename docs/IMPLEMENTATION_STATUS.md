...
| Inventory clearing and bill/receipt matching | GROUNDWORK_ONLY | `InventorySettings.inventoryClearingAccountId`, `InventoryPurchasePostingMode`, `PurchaseBill.inventoryPostingMode`, purchase bill preview, web receipt/bill/PO panels, `docs/inventory/*` | Inventory clearing account mapping, purchase receipt posting mode, enhanced receipt accounting preview, purchase bill receipt matching status, purchase order receipt/bill matching visibility, purchase bill direct/clearing accounting preview, and purchase receipt posting readiness compatibility counts. | No purchase receipt GL posting, no live bill clearing finalization, no landed cost, and no durable clearing reconciliation workflow. | Review clearing-mode bill finalization design with accountant, then add guarded receipt asset posting fields/endpoints. |
