...
| Bill finalization | COMPLETE_FOR_MVP | `purchase-bill-accounting.ts`, `FiscalPeriodGuardService` | Direct mode posts Dr expense/asset, Dr VAT receivable, Cr AP; explicit inventory-clearing mode posts tracked lines to Inventory Clearing with fiscal posting guard. | Receipt asset posting is separate/manual and only for compatible clearing-mode bills. | Add landed-cost, variance, and clearing reconciliation later. |
...
| COGS | PARTIAL | `apps/api/src/sales-stock-issues`, `InventorySettings`, web sales stock issue detail, `docs/inventory/*` | Sales stock issue detail/API can preview Dr COGS / Cr Inventory Asset, manually post one reviewed COGS journal, reverse it once, block stock issue void while active, and let P&L reflect posted journals naturally. | Manual only; no invoice auto-posting, FIFO, landed cost, returns workflow, or automatic inventory clearing reconciliation. | Harden accountant review workflow and add variance/return policy later. |
| Inventory clearing and bill/receipt matching | PARTIAL | `InventorySettings.inventoryClearingAccountId`, `InventoryPurchasePostingMode`, `PurchaseBill.inventoryPostingMode`, purchase bill preview/finalization, purchase receipt asset posting, web receipt/bill/PO panels, `docs/inventory/*` | Inventory clearing account mapping, purchase receipt posting mode, enhanced receipt accounting preview, purchase bill receipt matching status, purchase order receipt/bill matching visibility, purchase bill direct/clearing accounting preview, explicit clearing-mode bill finalization, explicit compatible receipt asset post/reverse journals, receipt void protection, and purchase receipt posting readiness compatibility counts. | No automatic purchase receipt GL posting, no direct-mode receipt posting, no landed cost, no durable clearing reconciliation workflow, and no migration for historical direct-mode bills. | Add clearing reconciliation, variance handling, and accountant reporting after manual receipt posting QA. |
...
| Purchase receiving | PARTIAL | `PurchaseReceipt`, `PurchaseReceiptLine`, `apps/api/src/purchase-receipts`, web `/inventory/purchase-receipts` | PO, purchase bill, and standalone receipts create `PURCHASE_RECEIPT_PLACEHOLDER` stock movements; source remaining quantity/status helpers; void reversal with negative-stock guard; enhanced accounting panel/API with bill matching values; compatible clearing-mode receipts can manually post/reverse Dr Inventory Asset / Cr Inventory Clearing journals; active asset posting blocks receipt void; permissions, tests, and smoke coverage. | No automatic receipt posting, no direct-mode receipt posting, no landed cost, supplier delivery documents, serial/batch tracking, or automatic bill/PO receipt. | Add clearing reconciliation, variance policy, landed-cost design, and historical migration/exclusion reporting. |
