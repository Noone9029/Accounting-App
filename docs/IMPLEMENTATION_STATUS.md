...
| Inventory settings | GROUNDWORK_ONLY | `InventorySettings`, `apps/api/src/inventory`, web `/inventory/settings` | Per-organization valuation method, negative-stock flag, value-tracking flag, inventory accounting enable flag, account mappings including inventory clearing, purchase receipt posting mode, validation, and manual COGS/purchase receipt no-posting warnings. | FIFO is placeholder only; enable flag permits manual COGS posting but does not auto-post accounting; purchase receipt posting mode remains non-posting. | Use settings as readiness control for future purchase receipt asset posting tasks. |
