...
| Module | Status | Files | What works | Gaps | Next step |
| --- | --- | --- | --- | --- | --- |
| Suppliers | COMPLETE_FOR_MVP | `Contact.type=SUPPLIER/BOTH`, `/suppliers`, supplier contact APIs | Dedicated Suppliers module under Purchases, supplier list/detail view, open/overdue payable balance, bills, purchase orders, supplier credits, payments, refunds, expenses, full AP transaction table, and report/workflow links. | Supplier onboarding fields are basic; no supplier bank detail model or import/merge workflow. | Add supplier tax/bank details and browser QA for AP row types. |
...
| Purchase bills | COMPLETE_FOR_MVP | `purchase-bills`, `purchase-matching` | Draft/edit/finalize/void, AP posting, PDF/archive, source purchase order link, read-only PO/bill/receipt matching visibility, supplier-grouped matching exception center, review workflow tracking, and local DEV-08/DEV-08F bill lifecycle evidence. | No multi-PO matching, production-grade partial matching policy, valuation variance booking, landed cost, or purchase returns integration. Review tracking exists but does not mutate bills or accounting. | Add accountant-reviewed matching tolerance policy and valuation-variance design after AP policy review. |
...
| Supplier ledger | COMPLETE_FOR_MVP | `supplierLedger`, contact page, `/suppliers` | AP balance and bill/payment/void rows plus supplier detail activity summary, purchase order activity, supplier credits/refunds, expenses, and supplier-specific payable wording. | Needs browser QA across every AP row type and stronger statement drill-down affordances. | Add AP row-type browser QA and supplier statement drill-down polish. |
