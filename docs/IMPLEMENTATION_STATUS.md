...
| Uploaded attachments | PARTIAL | `attachments`, `Attachment`, `AttachmentPanel`, key detail pages | JSON/base64 file upload, tenant-scoped linked-entity validation, metadata listing, download, notes update, soft delete, permissions, smoke checks, and reusable panels on sales, purchase, banking, and inventory detail pages. | Database/base64 storage only; no S3/object storage, virus scanning, OCR, retention policy, drag/drop polish, email sending, or ZATCA attachment submission. | Add object-storage provider, scanning policy, and OCR/import workflows after storage review. |
...
| Cloud storage | NOT_STARTED | Generated docs and uploaded attachments use DB | None. | PDF and uploaded file base64 in DB. | Add S3-compatible adapter. |
