...
| Supplier statement | COMPLETE_FOR_MVP | `/contacts/:id/supplier-statement`, `/contacts/:id/supplier-statement.pdf` | Date-filtered AP statement JSON and PDF export using existing supplier statement rows. | Supplier statement PDF does not create generated-document archive rows yet. | Add reviewed supplier statement archive tracking if required. |
