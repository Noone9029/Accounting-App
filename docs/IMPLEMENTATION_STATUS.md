...
| Inventory settings | GROUNDWORK_ONLY | `InventorySettings`, `apps/api/src/inventory`, web `/inventory/settings` | Per-organization valuation method, negative-stock flag, value-tracking flag, inventory accounting enable flag, account mappings, validation, and manual COGS readiness warnings. | FIFO is placeholder only; enable flag permits manual COGS posting but does not auto-post accounting; missing inventory clearing workflow blocks full inventory accounting. | Use settings as readiness control for future purchase receipt asset posting tasks. |
