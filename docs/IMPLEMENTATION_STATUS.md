...
| COGS | PARTIAL | `apps/api/src/sales-stock-issues`, `InventorySettings`, web sales stock issue detail, `docs/inventory/*` | Sales stock issue detail/API can preview Dr COGS / Cr Inventory Asset, manually post one reviewed COGS journal, reverse it once, block stock issue void while active, and let P&L reflect posted journals naturally. | Manual only; no invoice auto-posting, FIFO, landed cost, returns workflow, purchase receipt asset posting, or inventory clearing workflow. | Harden accountant review workflow and then design purchase receipt inventory asset posting. |
| Inventory clearing and bill/receipt matching | GROUNDWORK_ONLY | `InventorySettings.inventoryClearingAccountId`, `InventoryPurchasePostingMode`, `purchase-receipts`, web receipt/bill/PO panels, `docs/inventory/*` | Inventory clearing account mapping, purchase receipt posting mode, enhanced receipt accounting preview, purchase bill receipt matching status, purchase order receipt/bill matching visibility, and purchase receipt posting readiness audit endpoint/panel. | No purchase receipt GL posting, no bill clearing entries, no landed cost, and no durable clearing reconciliation workflow. | Resolve no-go audit items, then add guarded purchase receipt asset posting design. |
...
| Inventory settings | GROUNDWORK_ONLY | `InventorySettings`, `apps/api/src/inventory`, web `/inventory/settings` | Per-organization valuation method, negative-stock flag, value-tracking flag, inventory accounting enable flag, account mappings including inventory clearing, purchase receipt posting mode, validation, purchase receipt posting readiness panel, and manual COGS/purchase receipt no-posting warnings. | FIFO is placeholder only; enable flag permits manual COGS posting but does not auto-post accounting; purchase receipt posting mode remains non-posting. | Use readiness as an audit gate before future purchase receipt asset posting tasks. |
