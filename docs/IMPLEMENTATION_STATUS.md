...
| Email outbox and provider routing | GROUNDWORK_ONLY | `EmailOutbox`, `EmailSenderDomainEvidence`, `EmailProviderEvent`, `AuthToken`, `AuthTokenRateLimitEvent`, `apps/api/src/email`, `/settings/email-outbox` | Mock remains default; invite/password reset/test-send records are stored in outbox; `GET /email/readiness` reports provider/SMTP production readiness, sender-domain SPF/DKIM/DMARC evidence status, relay diagnostics, retry counts/processor flag, and bounce/provider-event/monitoring blockers without secrets; `POST /email/diagnostics` and default `POST /email/retry-process` return no-send/no-mutation skipped plans; SMTP adapter can be enabled by env and records `SENT_PROVIDER`; invite/reset delivery is DB-rate-limited; mock provider events can capture metadata-only bounce/delivery evidence. | No scheduled retry worker, signed production provider webhooks, suppression list, live DNS/provider validation, real relay execution evidence, monitoring, or paid/provider-specific API adapter. | Run an allowlisted non-production relay diagnostic, then add signed provider webhooks, suppression handling, and a scheduled retry worker before real customer use. |
...
| Email sending | GROUNDWORK_ONLY | `apps/api/src/email`, `EmailOutbox`, `EmailSenderDomainEvidence`, `EmailProviderEvent`, `AuthTokenRateLimitEvent`, `/settings/email-outbox`, `docs/email/*` | Mock/local organization invite and password reset email records are stored and inspectable by default; readiness endpoint and UI show production SMTP readiness, metadata-only SPF/DKIM/DMARC evidence, relay diagnostics status, retry processor/counts, mock-only provider event status, and no-customer-email safeguards without secrets; disabled-by-default diagnostics and retry processing return skipped/no-send/no-mutation status; `POST /email/test-send` still exercises the active provider; DB-backed invite/reset rate limits and manual expired-token cleanup exist; SMTP can send only when explicitly configured. | No invoice/statement send, scheduled retry worker, signed provider webhooks, suppression list, live DNS/provider validation, real relay execution evidence, or monitoring. | Run a non-production relay diagnostic with an allowlisted sandbox recipient, then add signed webhook verification, suppression handling, monitoring, and scheduled retry execution. |
