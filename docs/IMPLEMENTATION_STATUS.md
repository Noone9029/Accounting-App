...
| Bank statement import and reconciliation | PARTIAL | `bank-statements`, `bank-reconciliations`, `BankStatementImport`, `BankStatementTransaction`, `BankReconciliation`, `BankReconciliationItem`, web statement/reconciliation routes | Local JSON/CSV row import, statement transaction records, candidate lookup against posted bank journal lines, manual match, categorize-to-journal, ignore, reconciliation summary, draft reconciliation creation, zero-difference close, item snapshots, closed-period statement locks, and administrative void/unlock. | No real file upload parser, OFX/CAMT/MT940, auto-match, formal reconciliation report PDF, statement attachments, approval workflow, or live feeds. | Add reconciliation report PDF, reviewer approval workflow, and file-format import samples. |
