...
| Purchase receiving | GROUNDWORK_ONLY | `PurchaseReceipt`, `PurchaseReceiptLine`, `apps/api/src/purchase-receipts`, web `/inventory/purchase-receipts` | PO, purchase bill, and standalone receipts create `PURCHASE_RECEIPT_PLACEHOLDER` stock movements; source remaining quantity/status helpers; void reversal with negative-stock guard; permissions, tests, and no-journal behavior. | No inventory asset journal, landed cost, supplier delivery documents, serial/batch tracking, or automatic bill/PO receipt. | Add receiving QA, purchase matching hardening, then valuation accounting design. |
| Sales stock issue | GROUNDWORK_ONLY | `SalesStockIssue`, `SalesStockIssueLine`, `apps/api/src/sales-stock-issues`, web `/inventory/sales-stock-issues` | Finalized-invoice stock issue creates `SALES_ISSUE_PLACEHOLDER` movements; invoice remaining quantity/status helper; availability guard; void reversal; permissions, tests, and no-journal behavior. | No COGS journal, delivery document, serial/batch tracking, or automatic invoice stock issue. | Add delivery workflow and COGS posting design later. |
| Inventory settings | GROUNDWORK_ONLY | `InventorySettings`, `apps/api/src/inventory`, web `/inventory/settings` | Per-organization valuation method, negative-stock flag, and value-tracking flag with operational warnings. | FIFO is placeholder only; setting does not post accounting. | Use settings when COGS/valuation accounting is designed. |
