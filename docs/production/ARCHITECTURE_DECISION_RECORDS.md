...
| [ADR-013 API hosting decision](adrs/ADR-013-api-hosting-decision.md) | Decide production API runtime and worker-hosting direction separate from the current Vercel beta wrapper. | AWS ECS Fargate; AWS Elastic Beanstalk; DigitalOcean App Platform; AWS App Runner; Render/Fly/Railway-style managed API hosting; Vercel API Functions. | NestJS/Express runtime fit, container support, worker separation, RDS/PostgreSQL connectivity, Redis/queue connectivity, secrets, health checks, rollback, monitoring/logging, cost, operational load, ZATCA/email readiness. | Proposed: AWS ECS Fargate for paid SaaS v1 API with API and worker as separate services. DigitalOcean App Platform is secondary fallback, Elastic Beanstalk is AWS fallback only, App Runner is not recommended because official AWS docs say it is closed to new customers as of April 30, 2026, Render/Fly/Railway are backup/private-beta only, and Vercel remains beta/user-testing/staging only. Implementation has not started; no API service, worker service, ECS/Fargate config, env var, database, Redis, storage, ZATCA, email, customer data, or production deploy changed. | backend/infra/security/ops | paid private beta | proposed |
...
- ADR-001 and ADR-013 are drafted/proposed only; they did not perform Supabase/Vercel env changes, migrations, backups, app tests, real email, ZATCA actions, provider provisioning, ECS/Fargate configuration, worker hosting configuration, or production deploys.
