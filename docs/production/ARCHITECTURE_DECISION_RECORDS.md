...
| --- | --- | --- | --- | --- | --- | --- | --- |
| ADR-001 Final production hosting provider | Choose final production hosting separate from current Vercel beta/user-testing deployment. | Vercel plus managed workers; container platform; managed PaaS; cloud VM/container service. | API runtime fit, worker support, rollback, logs, secrets, DB connectivity, cost, support maturity, operational load. | Pending. Do not assume current Vercel beta hosting is final production hosting. | infra/product/security | paid private beta | pending |
