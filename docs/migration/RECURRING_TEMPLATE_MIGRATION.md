...
The importer never activates imported templates. It creates one draft template per row; journal rows carry primary and counter accounts so the normal balance validation remains authoritative. The accountant migration workspace exposes all four recurring template types and a separate run-history export. Template downloads and exports use authenticated browser requests with the active organization header. Template and run exports are tenant-scoped, CSV-injection protected, and capped at 10,000 rows.
