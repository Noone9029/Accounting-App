...
| `/settings/email-outbox` | Email provider readiness, safe diagnostics, sender-domain evidence, retry readiness, provider event/webhook readiness, suppression controls, and email outbox. | `/email/readiness`, `/email/diagnostics`, `/email/retry-plan`, `/email/provider-events/plan`, `/email/provider-events/webhook-plan`, `/email/suppressions`, `/email/sender-domain-evidence`, `/email/outbox`, selected detail, and optional `/auth/tokens/cleanup-expired`. | Review production readiness, SPF/DKIM/DMARC evidence status, relay diagnostics status, retry pending/blocked/suppressed counts, retry processor disabled/enabled state, signed-webhook status, suppression count/list, provider event status, redacted SMTP config booleans, warnings/blockers, mock/local/provider emails, disabled-by-default diagnostics, plan-only retry/event/webhook controls, metadata-only evidence create/verify/revoke controls, metadata-only suppression create/revoke controls, and expired-token cleanup when permitted. | Groundwork | SMTP adapter is opt-in; diagnostics and retry processing send no customer email by default; sender-domain evidence is manual metadata only; webhook verification is disabled by default and provider-agnostic/test-only; no scheduled worker, provider-specific production webhook adapter, live DNS/provider validation, real alerting, or monitoring. |
