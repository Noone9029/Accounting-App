...
| `/inventory/settings` | Inventory settings. | Inventory settings, inventory accounting settings, and accounts when allowed. | Review/update valuation method, negative-stock flag, inventory value tracking, inventory accounting enable flag, account mappings, and manual COGS posting warnings. | Groundwork | Settings enable manual COGS readiness only; they do not auto-post inventory journals or implement inventory clearing. |
