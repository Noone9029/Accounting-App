...
| `/inventory/settings` | Inventory settings. | Inventory settings, inventory accounting settings, and accounts when allowed. | Review/update valuation method, negative-stock flag, inventory value tracking, inventory accounting enable flag, account mappings including inventory clearing, purchase receipt posting mode, and manual COGS/purchase receipt no-posting warnings. | Groundwork | Settings enable manual COGS readiness and receipt previews only; they do not auto-post inventory journals or clearing entries. |
...
| `/purchases/bills/[id]` | Purchase bill detail. | Bill, lines, allocations, PDF data, source PO link, receiving status, and receipt matching status. | Finalize, void, delete draft, PDF, supplier ledger link, receive remaining tracked finalized lines when allowed, and review matched quantity/value warnings. | Implemented | No multi-PO clearing workflow or accounting mutation from matching. |
