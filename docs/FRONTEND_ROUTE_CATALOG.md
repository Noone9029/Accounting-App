...
| `/inventory/variance-proposals/[id]` | Inventory variance proposal detail. | Proposal, accounting preview, source bill/receipt, events. | Submit, approve, explicitly post, reverse, or void based on status and permissions; review Dr/Cr preview and timeline. | Implemented | Posting remains manual only; no automatic report-driven posting. |
