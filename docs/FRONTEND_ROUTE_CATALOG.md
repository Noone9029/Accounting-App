...
| `/inventory/settings` | Inventory settings. | Inventory settings, inventory accounting settings, purchase receipt posting readiness, and accounts when allowed. | Review/update valuation method, negative-stock flag, inventory value tracking, inventory accounting enable flag, account mappings including inventory clearing, purchase receipt posting mode, readiness blockers/warnings, and manual COGS/purchase receipt no-posting warnings. | Groundwork | Settings enable manual COGS readiness and receipt previews only; the readiness panel is advisory and no purchase receipt post button exists. |
