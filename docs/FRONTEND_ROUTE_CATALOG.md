...
| `/settings/email-outbox` | Email provider readiness, safe diagnostics, sender-domain evidence, retry readiness, provider event readiness, and email outbox. | `/email/readiness`, `/email/diagnostics`, `/email/retry-plan`, `/email/provider-events/plan`, `/email/sender-domain-evidence`, `/email/outbox`, selected detail, and optional `/auth/tokens/cleanup-expired`. | Review production readiness, SPF/DKIM/DMARC evidence status, relay diagnostics status, retry pending/blocked counts, retry processor disabled/enabled state, mock-only provider event status, redacted SMTP config booleans, warnings/blockers, mock/local/provider emails, disabled-by-default diagnostics, plan-only retry/event controls, metadata-only evidence create/verify/revoke controls, and expired-token cleanup when permitted. | Groundwork | SMTP adapter is opt-in; diagnostics and retry processing send no customer email by default; sender-domain evidence is manual metadata only; provider event ingestion is mock-only/unsigned; no scheduled worker, signed production webhooks, suppression list, live DNS/provider validation, or monitoring. |
