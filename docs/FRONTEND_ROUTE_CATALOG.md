...
- Settings/Admin nav now includes Team Members for `users.view`, Roles & Permissions for `roles.view`, Document settings for `documentSettings.view`, Storage for `documentSettings.view` or `attachments.manage`, Data management for `documentSettings.view` or `auditLogs.view`, Email outbox/readiness for `emailOutbox.view`, Audit logs for `auditLogs.view`, and Number sequences for `numberSequences.view`; audit CSV export/retention controls, backup/restore evidence controls, and number-sequence editing are additionally gated by `auditLogs.export`, `auditLogs.manageRetention`, and `numberSequences.manage`.
...
| `/bank-accounts/[id]/statement-imports` | Local statement import batches. | Bank profile, import batches, preview results. | Upload or paste CSV, JSON, OFX, CAMT XML, or MT940 text, preview validation/totals/warnings, import valid rows with optional partial import, void unmatched imports when allowed. | Implemented | Manual import only; no live bank feed, external aggregation, automatic matching, raw-file body archive implementation, or certified bank-specific parser coverage. |
...
| `/settings/storage` | Storage readiness, migration planning, and backup/restore readiness. | `/storage/readiness`, `/storage/migration-plan`, `/system/backup-readiness`, `/system/restore-drill-plan`, and `/system/backup-evidence` when the user has `auditLogs.manageRetention`. | Review active providers, redacted S3 config checks, database/S3 readiness warnings, target provider, dry-run migration counts, backup/restore blockers, restore-drill plan, evidence completeness, and metadata-only evidence create/verify/revoke controls. | Groundwork | Uploaded-attachment S3 adapter exists behind env config, but no migration executor, generated-document S3 storage, virus scanning, lifecycle/retention policy, real-bucket validation, real backup execution, or restore execution is implemented. |
| `/settings/data-management` | Data import/export and backup planning surface. | None. | Review read-only export, import, backup, restore, storage, and audit boundaries with links to existing evidence routes. | Planning | No export job, import parser, backup execution, restore execution, storage-provider switch, document rewrite, or hosted mutation. |
| `/settings/email-outbox` | Email provider readiness, safe diagnostics, sender-domain evidence, retry worker readiness, monitoring evidence, provider event/webhook readiness, suppression controls, and email outbox. | `/email/readiness`, `/email/diagnostics`, `/email/retry-plan`, `/email/retry-worker/plan`, `/email/monitoring-plan`, `/email/monitoring-evidence`, `/email/provider-events/plan`, `/email/provider-events/webhook-plan`, `/email/suppressions`, `/email/sender-domain-evidence`, `/email/outbox`, selected detail, and optional `/auth/tokens/cleanup-expired`. | Review production readiness, SPF/DKIM/DMARC evidence status, relay diagnostics status, retry pending/blocked/suppressed counts, retry processor and worker disabled/enabled state, monitoring evidence status, bounce/complaint threshold blockers, suppression trend/webhook health blockers, signed-webhook status, suppression count/list, provider event status, redacted SMTP config booleans, warnings/blockers, mock/local/provider emails, disabled-by-default diagnostics, plan-only retry/worker/event/webhook/monitoring controls, metadata-only evidence create/verify/revoke controls, metadata-only suppression create/revoke controls, and expired-token cleanup when permitted. | Groundwork | SMTP adapter is opt-in; diagnostics, retry processing, and worker runs send no customer email by default; sender-domain and monitoring evidence are manual metadata only; webhook verification is disabled by default and provider-agnostic/test-only; no production scheduler, provider-specific production webhook adapter, live DNS/provider validation, real alerting, or external monitoring integration. |
...
| `/settings/zatca` | ZATCA settings, readiness, checklist, SDK readiness, and hash-chain status. | Profile, EGS units, logs, adapter config, readiness, checklist, SDK readiness including execution flag, Java, required range, config/work-dir blockers, hash mode, metadata counts, SDK-mode enablement blockers, and reset-plan dry-run summary. | Update profile, create/update EGS, CSR, mock CSID, dry-run/local SDK readiness visibility, review no-mutation hash-chain reset risks, and explicitly enable SDK hash mode on an eligible fresh EGS with reason/confirmation when permitted. | Groundwork only | Real ZATCA onboarding/submission not present; official samples pass locally under Java 11, LedgerByte standard fixture passes global validation, simplified fixture still needs signing/QR/certificate work, SDK hash persistence is fresh-EGS opt-in only, and execution remains disabled by default. |
...
| `/sales/invoices/[id]` | Invoice detail. | Invoice, payments, credit notes, allocations, stock issue status, ZATCA metadata, EGS/metadata hash modes, stored hash, local SDK validation/hash comparison results, PDFs, and linked attachments. | Finalize, void, delete draft, PDF download, ZATCA local actions, SDK dry-run/local validation, no-mutation SDK hash comparison, create credit note, issue stock for remaining tracked finalized lines when allowed, and manage supporting attachments. | Implemented | ZATCA actions remain local/mock only; no production compliance, signing, submission, or live official SDK execution by default. |
