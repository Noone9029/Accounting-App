...
| `/purchases/bills/new` | Create purchase bill. | Suppliers, branches, accounts, tax rates, items, and inventory posting mode. | Save draft in direct mode by default or select inventory-clearing preview mode after accountant review. | Implemented | Vendor attachments missing; clearing mode cannot be finalized yet. |
| `/purchases/bills/[id]` | Purchase bill detail. | Bill, lines, allocations, PDF data, source PO link, receiving status, receipt matching status, and accounting preview. | Finalize direct-mode drafts, void, delete draft, PDF, supplier ledger link, receive remaining tracked finalized lines when allowed, review matched quantity/value warnings, and review direct vs clearing-mode preview journal lines. | Implemented | No multi-PO clearing workflow or accounting mutation from matching; clearing-mode finalization is blocked. |
| `/purchases/bills/[id]/edit` | Edit draft bill. | Bill and form dependencies, including inventory posting mode. | Save draft changes and choose direct or clearing preview mode. | Implemented | Not available after finalize by design; clearing mode remains preview-only. |
