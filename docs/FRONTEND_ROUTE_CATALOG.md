...
| `/settings/email-outbox` | Email provider readiness, safe diagnostics, sender-domain evidence, and email outbox. | `/email/readiness`, `/email/diagnostics`, `/email/sender-domain-evidence`, `/email/outbox`, selected detail, and optional `/auth/tokens/cleanup-expired`. | Review production readiness, SPF/DKIM/DMARC evidence status, relay diagnostics status, redacted SMTP config booleans, warnings/blockers, mock/local/provider emails, disabled-by-default diagnostics, metadata-only evidence create/verify/revoke controls, and expired-token cleanup when permitted. | Groundwork | SMTP adapter is opt-in; diagnostics sends no customer email by default; sender-domain evidence is manual metadata only; no retries, bounce handling, background queue, provider webhooks, or live DNS/provider validation. |
