...
| `/settings/zatca` | ZATCA settings, readiness, checklist, SDK readiness. | Profile, EGS units, logs, adapter config, readiness, checklist, SDK readiness including execution flag, Java, required range, config/work-dir blockers. | Update profile, create/update EGS, CSR, mock CSID, dry-run/local SDK readiness visibility. | Groundwork only | Real ZATCA onboarding/submission not present; official samples pass locally under Java 11, LedgerByte fixtures now clear first structural SDK checks, and execution remains disabled by default. |
