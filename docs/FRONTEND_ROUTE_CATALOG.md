...
| `/inventory/purchase-receipts/[id]` | Purchase receipt detail. | Receipt, lines, source document, warehouse, movement links, and enhanced accounting preview. | Review stock movement links, receipt value, linked bill mode, matched bill value, value difference, matching summary, preview Dr Inventory Asset / Cr Inventory Clearing lines, warnings/blockers, manually post inventory asset with `inventory.receipts.postAsset` when eligible, reverse asset posting with `inventory.receipts.reverseAsset`, and void when allowed. | Partial | No edit after posting, no automatic posting, no direct-mode receipt posting, and no landed-cost workflow. |
...
| `/purchases/bills/[id]` | Purchase bill detail. | Bill, lines, allocations, PDF data, source PO link, receiving status, receipt matching status, and accounting preview. | Finalize direct-mode or compatible clearing-mode drafts, void, delete draft, PDF, supplier ledger link, receive remaining tracked finalized lines when allowed, review matched quantity/value warnings, receipt asset posted/reversed status, unposted clearing-mode receipt warning, and direct vs clearing-mode preview journal lines. | Implemented | No multi-PO clearing workflow, clearing reconciliation, direct-mode receipt posting, or accounting mutation from matching. |
