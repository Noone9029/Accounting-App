...
- Settings/Admin nav now includes Team Members for `users.view`, Roles & Permissions for `roles.view`, Document settings for `documentSettings.view`, Storage for `documentSettings.view` or `attachments.manage`, Email outbox/readiness for `emailOutbox.view`, and Audit logs for `auditLogs.view`.
...
| `/settings/audit-logs` | Audit log review. | `/audit-logs` and selected `/audit-logs/:id`. | Filter by action, entity type, actor, date, and search; review timestamp, actor, action, entity, summary, and sanitized before/after metadata. | Implemented | No export, alerting, immutable external store, or anomaly detection. |
...
| `/sales/invoices/[id]` | Invoice detail. | Invoice, payments, credit notes, allocations, stock issue status, ZATCA metadata, PDFs, and linked attachments. | Finalize, void, delete draft, PDF download, ZATCA local actions, create credit note, issue stock for remaining tracked finalized lines when allowed, and manage supporting attachments. | Implemented | Browser E2E and more polished error recovery needed. |
