...
- Settings/Admin nav now includes Team Members for `users.view`, Roles & Permissions for `roles.view`, Document settings for `documentSettings.view`, Storage for `documentSettings.view` or `attachments.manage`, Email outbox/readiness for `emailOutbox.view`, and Audit logs for `auditLogs.view`; audit CSV export and retention controls are additionally gated by `auditLogs.export` and `auditLogs.manageRetention`.
