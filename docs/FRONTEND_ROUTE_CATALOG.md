...
| `/settings/email-outbox` | Email provider readiness, safe diagnostics, and email outbox. | `/email/readiness`, `/email/diagnostics`, `/email/outbox`, selected detail, and optional `/auth/tokens/cleanup-expired`. | Review production readiness, redacted SMTP config booleans, warnings/blockers, mock/local/provider emails, disabled-by-default diagnostics, and expired-token cleanup when permitted. | Groundwork | SMTP adapter is opt-in; diagnostics sends no customer email by default; no retries, bounce handling, background queue, provider webhooks, or domain auth validation. |
