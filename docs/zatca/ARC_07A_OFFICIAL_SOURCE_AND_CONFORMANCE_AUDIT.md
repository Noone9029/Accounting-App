...
Unsigned XML validation is proven for the registered fixtures. LedgerByte C14N 1.1/invoice-hash parity is proven for every current LedgerByte-generated fixture. A configured Tier-2 test constructs LedgerByte XAdES signatures with externally supplied local-only dummy material, generates nine-tag Phase 2 QR values in LedgerByte code, and verifies the decoded QR ECDSA signature locally. The official SDK `-validate` command accepts temporary standard and Arabic simplified signed XML artifacts, but a local negative probe showed that it also accepts a modified SignedProperties timestamp. It is therefore schema/structure compatibility evidence, not independent cryptographic-signature validation. The SDK did not generate those signatures or QR values, and the tests retain no XML, QR, certificate, key, or raw SDK output. This is local compatibility evidence only; SDK validation is not ZATCA approval, and the CI readiness guard correctly remains `CI_BLOCKED_MISSING_SDK_REFERENCE` for a fresh checkout because no reproducible licensed SDK acquisition policy exists.
