...
Unsigned XML validation is proven for the registered fixtures. LedgerByte C14N 1.1/invoice-hash parity is proven for every current LedgerByte-generated fixture. LedgerByte now also independently verifies serialized local signed artifacts using secure namespace-aware DOM traversal, C14N 1.1 digest recomputation, X.509 certificate-byte and issuer/serial linkage, secp256k1 ECDSA verification, and nine-tag QR binding. The verifier rejects a changed `xades:SigningTime` with `SIGNED_PROPERTIES_INVALID`; it does not call the SDK validator and returns metadata-only safe status/codes and boolean checks. A configured Tier-2 test constructs LedgerByte XAdES signatures with externally supplied local-only dummy material and verifies valid standard and Arabic simplified artifacts through this verifier. The official SDK `-validate` command is then run separately as a structural oracle. The SDK did not generate those signatures or QR values, and the tests retain no XML, QR, certificate, key, or raw SDK output. This is local compatibility evidence only; it does not prove production certificate trust, KMS/HSM custody, ZATCA approval, or production compliance. The CI readiness guard correctly remains `CI_BLOCKED_MISSING_SDK_REFERENCE` for a fresh checkout because no reproducible licensed SDK acquisition policy exists.
