...
The refreshed metadata-only artifact records seven unique SDK validation artifacts: two official SDK samples and five LedgerByte-generated fixtures, including the new debit-note and document-allowance cases. Historical `ledgerbyte-generated-credit-note` is an alias of `ledgerbyte-credit-note` and is excluded from aggregates. Evidence reports registered/unique/official/LedgerByte-generated/valid/invalid counts, `networkCallsMade: false`, `productionComplianceEnabled: false`, and no XML, QR, private-key, token, or header output. The runner now resolves official samples from the explicitly configured SDK root rather than assuming that ignored material exists in a clean worktree.

Unsigned XML validation is proven for the registered fixtures. LedgerByte C14N 1.1/invoice-hash parity is proven for the original three LedgerByte-generated valid fixtures; it must be expanded to the debit-note and document-allowance fixtures. A separate configured Tier-2 test now constructs a LedgerByte XAdES signature with externally supplied local-only dummy material, generates its nine-tag Phase 2 QR in LedgerByte code, verifies the ECDSA signature locally from decoded QR material, and validates the temporary signed XML with the official offline SDK. The SDK did not generate that signature or QR, and the test retains no XML, QR, certificate, key, or raw SDK output. This is local compatibility evidence only; SDK validation is not ZATCA approval, and the CI readiness guard correctly remains `CI_BLOCKED_MISSING_SDK_REFERENCE` for a fresh checkout because no reproducible licensed SDK acquisition policy exists.
...
The current implementation map still identifies unimplemented or unverified areas: debit note, multiple VAT/zero/exempt cases, allowance/rounding cases, invalid-rule fixtures, a database-backed issuance proof, and a full production-safe signing implementation. C14N11/hash parity is proven only for the current three LedgerByte-generated fixtures and must expand with the fixture matrix. The local signing provider remains intentionally disabled except for explicit `LOCAL_TEST` external paths; it is not a production key-custody implementation. No live credential, network request, OTP, CSID, clearance, reporting, customer data, or persisted signed artifact was used in this audit.
