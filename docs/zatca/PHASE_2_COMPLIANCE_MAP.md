...
| Compliance CSID | `MOCK_ONLY` | `apps/api/src/zatca/adapters/mock-zatca-onboarding.adapter.ts`, `docs/zatca/KEY_CUSTODY_AND_CSID_LIFECYCLE_DESIGN.md`, `docs/zatca/CSID_LIFECYCLE_CHECKLIST.md` | Add the sandbox CSID preflight guard before any real OTP/CSID execution; replace legacy body persistence with custody-only handling before real sandbox use. | Sandbox access, OTP approval, endpoint, auth, response fields, custody approval. |
