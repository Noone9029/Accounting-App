...
| Compliance CSID | `MOCK_ONLY` | `apps/api/src/zatca/adapters/mock-zatca-onboarding.adapter.ts`, `docs/zatca/KEY_CUSTODY_AND_CSID_LIFECYCLE_DESIGN.md`, `docs/zatca/CSID_LIFECYCLE_CHECKLIST.md`, `docs/zatca/SANDBOX_CSID_PREFLIGHT_GUARD.md`, `docs/zatca/CSID_RESPONSE_CUSTODY_GUARD.md` | Use the custody guard result before any sandbox adapter approval; replace legacy body persistence with custody-only handling before real sandbox use. | Sandbox access, OTP approval, endpoint, auth, response fields, approved custody provider, adapter execution approval. |
