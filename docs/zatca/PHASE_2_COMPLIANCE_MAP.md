...
| Previous invoice hash | `DONE_LOCAL` | `apps/api/src/zatca/zatca.service.ts`, `packages/zatca-core/src/index.ts`, `apps/api/src/zatca/zatca.controller.ts` | Use the dry-run reset plan to review existing EGS ICV/last-hash state before any future switch to SDK-generated hashes. | Official hash-chain lifecycle and device/EGS sequencing rules. |
| Invoice hash | `SKELETON` | `packages/zatca-core/src/index.ts`, `apps/api/src/zatca-core.spec.ts`, `apps/api/src/zatca-sdk` | SDK `-generateHash` comparison is now available as a read-only local oracle; implement SDK/C14N11 persistence only after the no-mutation comparison and dry-run reset plan are reviewed. | Official canonicalization and hash algorithm samples. |
