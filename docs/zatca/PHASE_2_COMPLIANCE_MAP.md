...
| Invoice hash | `SKELETON` | `packages/zatca-core/src/index.ts`, `apps/api/src/zatca-core.spec.ts` | Replace blocked groundwork helper with SDK `-generateHash` oracle or a verified C14N11 implementation. | Official canonicalization and hash algorithm samples. |
...
| Official SDK local XML validation | `PARTIAL` | `apps/api/src/zatca-sdk`, `apps/web/src/app/(app)/settings/zatca/page.tsx`, invoice detail ZATCA panel, `docs/zatca/OFFICIAL_SDK_FIXTURE_VALIDATION_RESULTS.md` | Official SDK samples pass under Java 11; LedgerByte standard fixture now passes globally and simplified fixture passes XSD/EN/PIH but still fails expected signing/QR/certificate production checks. Next validate generated invoice XML and implement signing/hash prerequisites. | Java 11-14 runtime and verified local SDK paths for repeatable local/CI validation. |
