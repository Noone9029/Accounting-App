...
| UBL XML | `PARTIAL` | `packages/zatca-core/src/index.ts` | Add standard supply date mapping, canonical PIH/hash-chain behavior, and signed simplified invoice structure after official validation review. | Official XML implementation standard, SDK samples, and canonicalization/signature docs. |
| Official SDK local XML validation | `PARTIAL` | `apps/api/src/zatca-sdk`, `apps/web/src/app/(app)/settings/zatca/page.tsx`, invoice detail ZATCA panel, `docs/zatca/OFFICIAL_SDK_FIXTURE_VALIDATION_RESULTS.md` | Official SDK samples pass under Java 11; LedgerByte local fixtures now pass XSD/EN structure checks but still fail PIH/signature/QR production checks. Next validate generated invoice XML and implement hash/signature prerequisites. | Java 11-14 runtime and verified local SDK paths for repeatable local/CI validation. |
