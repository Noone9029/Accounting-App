...
| Compliance CSID | `MOCK_ONLY` | `apps/api/src/zatca/adapters/mock-zatca-onboarding.adapter.ts`, `docs/zatca/KEY_CUSTODY_AND_CSID_LIFECYCLE_DESIGN.md`, `docs/zatca/CSID_LIFECYCLE_CHECKLIST.md`, `docs/zatca/SANDBOX_CSID_PREFLIGHT_GUARD.md` | Use the preflight result to draft the sandbox OTP and compliance CSID approval plan; replace legacy body persistence with custody-only handling before real sandbox use. | Sandbox access, OTP approval, endpoint, auth, response fields, custody approval. |
