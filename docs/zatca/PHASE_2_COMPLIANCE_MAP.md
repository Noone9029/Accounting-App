...
| Previous invoice hash | `DONE_LOCAL` | `apps/api/src/zatca/zatca.service.ts`, `packages/zatca-core/src/index.ts` | Validate generated-invoice hash-chain sequencing and then connect it to signing flow. | Official hash-chain lifecycle and device/EGS sequencing rules. |
| Invoice hash | `SKELETON` | `packages/zatca-core/src/index.ts`, `apps/api/src/zatca-core.spec.ts`, `apps/api/src/zatca-sdk` | SDK `-generateHash` comparison is now available as a read-only local oracle; replace app hash-chain storage only after SDK/C14N11 hashing and metadata reset/migration policy are designed. | Official canonicalization and hash algorithm samples. |
