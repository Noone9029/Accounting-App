...
| Invoice hash | XML-aware C14N11 provider boundary, `computeZatcaInvoiceHash`, and SDK `fatoora -generateHash -invoice <filename>` | Hash input after removing `ext:UBLExtensions`, QR `AdditionalDocumentReference`, and `cac:Signature` | `NEEDS_OFFICIAL_VERIFICATION` | Yes | The legacy compatibility API now fails closed and performs no regex XML transforms. LedgerByte blocks official hash output until the verified C14N11 provider is used. Local SDK hash outputs are recorded for fixtures; app hash-chain behavior remains non-production. |
