...
| CSR/key algorithm/profile | ARC-07B-06C proves the LedgerByte-generated synthetic CSR and custody path. ARC-07B-06H separately proves official SDK Simulation CSR generation, independent LedgerByte inspection, and disposable DPAPI custody of the SDK-generated key. The 06H local oracle requires exact SDK JAR/configuration and five-component JDK pins, Windows handle leases, token-SID ACLs, pre-re-derivation plaintext-key removal, confirmed termination, and complete expected-digest cleanup. | SDK CSR config template defines required fields; SDK readme references EC secp256k1 private key handling for signing. API docs require CSR for CSID. The SDK does not document validation of an existing CSR or ingestion of an existing private key. | `packages/zatca-core/src/index.ts`, `apps/api/src/zatca/zatca.service.ts`, `apps/api/src/zatca/custody/`. | Preserve the separate structural and cryptographic proofs; do not compare the independently generated 06C and 06H CSR/key pairs for equality. Keep real onboarding blocked pending its standalone approval, OTP, and network gates. |
