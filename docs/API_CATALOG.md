...
| GET | `/inventory/purchase-receipt-posting-readiness` | Purchase receipt posting readiness audit | Yes | Yes | Groundwork | Requires `inventory.view`; returns advisory ready/canEnablePosting flags, blockers, warnings, Inventory Asset and Inventory Clearing account mappings, and recommended next step; creates no settings, journals, or accounting mutations and does not enable purchase receipt GL posting. |
