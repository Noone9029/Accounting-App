...
| POST | `/sales-invoices/:id/zatca/sdk-validate` | Run local SDK validation for generated invoice XML | Yes | Yes | Groundwork | Disabled by default; no network/submission/compliance marking; Java 11 local fixture validation is documented, but generated invoice validation remains pending. |
...
| POST | `/zatca-sdk/validate-reference-fixture` | Local SDK validation for allowlisted fixture XML | Yes | Yes | Groundwork | Only `reference/` and `packages/zatca-core/fixtures`; path traversal blocked; default smoke expects disabled local-only response; official samples pass under Java 11 and LedgerByte local fixtures now pass first structural checks but still fail PIH/signature/QR production checks. |
