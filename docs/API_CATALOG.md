...
| Method | Path | Purpose | Auth | Org header | Status | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| GET | `/automation/proposal-boundary` | Automation and AI proposal boundary | Yes | Yes | Groundwork | Requires `dashboard.view`; read-only/no-mutation; reports proposal-only mode, human-confirmation and audit requirements, and blocked automatic posting/provider/hosted/compliance actions without executing automation, sending email, mutating providers, or making production-readiness claims. |
...
| Method | Path | Purpose | Auth | Org header | Status | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| GET | `/email/readiness` | Email provider readiness | Yes | Yes | Implemented | Requires `emailOutbox.view` or `users.manage`; read-only/no-mutation; reports SMTP config booleans, diagnostics plan, sender-domain SPF/DKIM/DMARC evidence status, relay diagnostics status, retry counts/processor flag, webhook verification booleans, suppression counts, bounce/provider-event/alerting/monitoring gaps, warnings, blockers, and redaction guarantees without exposing secrets. |
| GET | `/email/diagnostics-plan` | Safe email diagnostics plan | Yes | Yes | Implemented | Requires `users.manage`; read-only/no-mutation; reports diagnostics execution gate, allowlist configuration, provider label, redacted planned recipient, no-customer-email/no-mutation defaults, and `productionReady=false`. |
| POST | `/email/diagnostics` | Safe email diagnostics | Yes | Yes | Implemented | Requires `users.manage`; disabled by default, sends no customer email, creates no outbox record, and only attempts an allowlisted non-customer diagnostic recipient when explicitly enabled. |
| GET | `/email/invoice-payment/readiness` | Invoice/payment email delivery readiness | Yes | Yes | Groundwork | Requires `users.manage`; reports `NONE`, `MOCK_EMAIL`, `DISABLED_PROVIDER_PLACEHOLDER`, or `FUTURE_SMTP_OR_PROVIDER` state, supported invoice/payment templates, preview availability, blocked actual sending, no provider calls, no credential storage, and redaction guarantees. |
| POST | `/email/invoice-payment/preview` | Local invoice/payment email preview | Yes | Yes | Groundwork | Requires `users.manage`; only works when `LEDGERBYTE_INVOICE_PAYMENT_EMAIL_PROVIDER=MOCK_EMAIL`; renders fake/local invoice/payment template data, stores a redacted `EmailDeliveryEvent` with requestId, sends no email, and calls no provider. |
| POST | `/email/invoice-payment/delivery-blocked` | Record blocked invoice/payment delivery attempt | Yes | Yes | Groundwork | Requires `users.manage`; records a blocked metadata event for invoice/payment email delivery attempts, stores redacted recipient/requestId only, sends no email, creates no outbox record, and calls no provider. |
| GET | `/email/retry-plan` | Email retry plan | Yes | Yes | Implemented | Requires `users.manage`; read-only/no-mutation; reports pending, retryable failed, blocked, suppressed, active-suppression, and due retry counts plus max-attempts policy without sending email. |
| POST | `/email/retry-process` | Email retry processor | Yes | Yes | Implemented | Requires `users.manage`; disabled by default with `LEDGERBYTE_EMAIL_RETRY_PROCESSOR_ENABLED=false`; default response skips without sending email or mutating, and enabled runs update existing outbox retry metadata only while blocking matched active suppressions without provider calls. |
| GET | `/email/retry-worker/plan` | Scheduled retry worker plan | Yes | Yes | Implemented | Requires `users.manage`; read-only/no-mutation; reports worker gate state, scheduler provider `NONE` by default, due retry count, suppressed count, active suppression count, max-attempts policy, recommended cadence, and blockers without sending email. |
| POST | `/email/retry-worker/run` | Retry worker execution shell | Yes | Yes | Groundwork | Requires `users.manage`; disabled by default with `LEDGERBYTE_EMAIL_RETRY_WORKER_ENABLED=false`; default response skips without sending or mutating, and enabled runs still require the retry processor gate before processing existing due records. |
| GET | `/email/monitoring-plan` | Email monitoring evidence plan | Yes | Yes | Implemented | Requires `users.manage`; read-only/no-mutation; reports retry throughput, bounce/complaint alert threshold, suppression trend, delivery dashboard, and provider webhook health evidence gaps with `productionReadyContribution=false` until verified. |
| GET | `/email/monitoring-evidence` | List monitoring evidence | Yes | Yes | Implemented | Requires `users.manage`; tenant-scoped metadata-only list of delivery monitoring evidence; sends no email and returns no SMTP/API/webhook secrets, raw payloads, recipients, or message bodies. |
| POST | `/email/monitoring-evidence` | Create monitoring evidence draft | Yes | Yes | Implemented | Requires `users.manage`; stores metadata-only monitoring evidence and rejects SMTP/API/webhook secrets, auth headers, raw provider payloads, customer recipient lists, and customer message bodies. |
| POST | `/email/monitoring-evidence/:id/verify` | Verify monitoring evidence | Yes | Yes | Implemented | Requires `users.manage`; marks reviewed metadata as verified; can contribute only to monitoring readiness, not full production readiness while relay/provider/webhook/worker gates remain blocked. |
| POST | `/email/monitoring-evidence/:id/revoke` | Revoke monitoring evidence | Yes | Yes | Implemented | Requires `users.manage`; revokes metadata evidence without sending email, creating outbox records, or exposing secrets. |
| GET | `/email/provider-events/plan` | Provider event readiness | Yes | Yes | Implemented | Requires `users.manage`; read-only/no-mutation; reports event ingestion, signed-webhook status, bounce/suppression/alerting/monitoring blockers, and `productionReadyContribution=false`. |
| POST | `/email/provider-events/mock` | Mock provider event capture | Yes | Yes | Implemented | Requires `users.manage`; captures metadata-only unsigned local/mock provider events, rejects secrets/customer message content, sends no email, and does not make production monitoring ready. |
| GET | `/email/provider-events/webhook-plan` | Signed provider webhook plan | Yes | Yes | Implemented | Requires `users.manage`; read-only/no-mutation; reports disabled-by-default webhook verification, webhook-secret and allowed-provider booleans, verified-event count, no raw headers, no raw payload, no webhook secret, and `productionReadyContribution=false` unless signed evidence exists. |
| POST | `/email/provider-events/webhook` | Signed provider webhook ingestion | Yes | Yes | Groundwork | Requires `users.manage`; disabled by default; rejects/skips unverified input without persistence/mutation; when explicitly enabled, accepts only allowlisted provider-agnostic HMAC test signatures and stores redacted metadata only. |
| GET | `/email/suppressions` | List email suppressions | Yes | Yes | Implemented | Requires `users.manage`; tenant-scoped metadata-only list of masked/hash suppressions; sends no email and returns no raw suppression email or provider secrets. |
| POST | `/email/suppressions` | Create email suppression | Yes | Yes | Implemented | Requires `users.manage`; accepts an email only as request input, stores masked/hash metadata, sends no email, creates no outbox record, and rejects secret/customer-content metadata. |
| POST | `/email/suppressions/:id/revoke` | Revoke email suppression | Yes | Yes | Implemented | Requires `users.manage`; metadata-only revoke of active suppression with no email send and no secret exposure. |
| GET | `/email/sender-domain-evidence` | List sender-domain evidence | Yes | Yes | Implemented | Requires `users.manage`; tenant-scoped metadata-only evidence list; sends no email and returns no SMTP/API/provider secrets. |
| POST | `/email/sender-domain-evidence` | Create sender-domain evidence draft | Yes | Yes | Implemented | Requires `users.manage`; stores SPF/DKIM/DMARC/MX/return-path/provider-verification metadata only; rejects private DKIM keys, SMTP passwords, API keys, tokens, auth headers, connection URLs, and provider secrets. |
| POST | `/email/sender-domain-evidence/:id/verify` | Verify sender-domain evidence | Yes | Yes | Implemented | Requires `users.manage`; marks reviewed metadata as verified; can contribute only to sender-domain review, not full production readiness while relay/bounce/retry/monitoring gaps remain. |
| POST | `/email/sender-domain-evidence/:id/revoke` | Revoke sender-domain evidence | Yes | Yes | Implemented | Requires `users.manage`; revokes metadata evidence without sending email or creating outbox records. |
...
| GET | `/contacts/:id/supplier-statement.pdf` | Supplier statement PDF | Yes | Yes | Implemented | Archives PDF on download as `SUPPLIER_STATEMENT`. |
...
| POST | `/accounting-close/cycles/:id/prepare`, `/review`, `/return-to-preparer` | Advance preparer/reviewer workflow | Implemented | Return requires a reason while the fiscal period is open. A closed cycle can return only after the separate authorized fiscal-period reopen workflow has reopened its period. |
| POST | `/accounting-close/cycles/:id/close` | Close the fiscal period through the authoritative workflow | Implemented | Requires `accountingClose.close`, optimistic version, and `Idempotency-Key` (8-128 safe characters). Revalidates readiness inside the serializable fiscal-period transition; duplicate matching keys replay the safe recorded result without another close. |
| POST | `/accounting-close/cycles/:id/lock` | Lock the fiscal period through the authoritative workflow | Implemented | Requires `accountingClose.lock`, optimistic version, and `Idempotency-Key` (8-128 safe characters). Revalidates readiness inside the serializable fiscal-period transition; post-close drift writes a frozen snapshot and safe conflict without locking; duplicate matching keys replay the recorded result without another lock. |
