...
| GET | `/email/retry-plan` | Email retry plan | Yes | Yes | Implemented | Requires `users.manage`; read-only/no-mutation; reports pending, retryable failed, blocked, and due retry counts plus max-attempts policy without sending email. |
| POST | `/email/retry-process` | Email retry processor | Yes | Yes | Implemented | Requires `users.manage`; disabled by default with `LEDGERBYTE_EMAIL_RETRY_PROCESSOR_ENABLED=false`; default response skips without sending email or mutating, and enabled runs update existing outbox retry metadata only. |
| GET | `/email/provider-events/plan` | Provider event readiness | Yes | Yes | Implemented | Requires `users.manage`; read-only/no-mutation; reports mock-only event ingestion, unsigned webhook status, bounce/monitoring blockers, and `productionReadyContribution=false`. |
