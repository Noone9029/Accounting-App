-- Local review artifact; not evidence of execution. Target only the approved beta project.
-- Target project: xynelbjqcmbgtscfmmzv. Verify the Supabase tool project_id before execution.
-- Generated expectations from untouched baseline 90e0eaa4896a55c1c8cda1c4101f4ab1323a4a61 migration SQL.
-- Read-only: exactly one SELECT, catalog metadata only. Require status=PASS and issueCount=0.
-- Covers exact types/typmods/nullability/defaults, enums, indexes, PK/check/FK semantics and effective browser-role access.
-- Extra unrelated columns/objects are not rejected. RLS is reported, not blindly enabled.
WITH
