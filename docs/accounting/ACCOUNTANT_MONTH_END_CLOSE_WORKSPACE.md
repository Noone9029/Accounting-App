...
A reviewed cycle may be explicitly returned to `IN_PROGRESS` while its fiscal period is still open. That return requires an optimistic version and a reason, clears the active preparer/reviewer identities and readiness hash, preserves the historical reviewed snapshot, and retains the immutable recorded sign-off mode. Sign-off modes are `SEPARATED` by default or `SINGLE_USER_DEMO` only when the organization’s default-off policy is explicitly enabled and exactly one active member is eligible to review. If the final close recheck detects a stale review, the workspace commits the same invalidated state with a new immutable `DRAFT` readiness snapshot before returning a conflict; it never closes the fiscal period from stale evidence.
