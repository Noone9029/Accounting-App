...
- No production email queue/webhook security controls, executed relay validation, or live domain authentication validation.
...
- Setup wizard and checklist now exist; deeper onboarding still needs browser E2E, templates, and guided empty-state polish.
...
- Added a richer `GET /email/readiness` response for provider configuration, from/reply-to configuration, SMTP booleans, `productionReady`, blockers, warnings, and redaction guarantees. The endpoint remains read-only and sends no email.
- Added `POST /email/diagnostics`, disabled by default via `LEDGERBYTE_EMAIL_DIAGNOSTICS_SEND_ENABLED=false`. Default diagnostics returns a skipped plan with `noEmailSent=true`, `noCustomerEmailSent=true`, and `noMutation=true`.
- Optional diagnostics execution requires an explicit allowlisted test recipient and returns a redacted delivery summary. It does not persist outbox records and does not return SMTP passwords, usernames, API keys, tokens, URLs, authorization headers, or provider secrets.
- `/settings/email-outbox` now shows production email readiness, password-reset/invite reliability warnings, and diagnostics disabled-by-default messaging.
- Production email remains blocked until SMTP is validated against a non-production relay with sender-domain authentication, retry/bounce/webhook handling, and monitoring.

## 2026-05-19 Email sender-domain readiness update

- Added `EmailSenderDomainEvidence` for tenant-scoped metadata-only SPF, DKIM, DMARC, MX, return-path, and provider-verification evidence.
- Added evidence list/create/verify/revoke endpoints and settings-page controls. They do not send email, do not create outbox records, and reject SMTP passwords, API keys, tokens, authorization headers, connection URLs, provider secrets, private DKIM keys, and customer email content.
- `GET /email/readiness` now reports sender-domain evidence status, missing/verified SPF/DKIM/DMARC, relay diagnostics status, and explicit false values for bounce webhooks, retry policy, and monitoring.
- Verified sender-domain evidence can move the domain section to ready-for-review, but full email `productionReady` remains false until relay diagnostics, bounce/retry/monitoring, and provider review are complete.

## 2026-05-19 Email retry and bounce readiness update

- Added durable retry fields to `EmailOutbox` and a disabled-by-default `POST /email/retry-process`; default retry processing returns skipped/no-send/no-mutation status.
- Added read-only `GET /email/retry-plan` for pending/retryable/blocked/due counts and max-attempts policy.
...
- Deployment/storage readiness improved because LedgerByte now has metadata-only `BackupRestoreEvidence`, read-only `/system/backup-readiness`, read-only `/system/restore-drill-plan`, and backup evidence list/create/verify/revoke endpoints.
- The storage settings page now shows database backup, point-in-time recovery, migration-history, object-storage backup, generated-document backup, attachment backup, restore-drill, restore-verification, and RPO/RTO review evidence state without any run-backup or restore action.
- Evidence capture rejects database URLs, Supabase service role keys, storage credentials, SMTP/provider secrets, auth headers, private keys, signed XML/QR bodies, customer document contents, and attachment contents.
- The score remains limited because no real Supabase/Postgres backup or PITR setting was verified, no object-storage backup policy was exercised, no restore drill was executed, and RPO/RTO plus retention requirements still need business review.
