...
## Saudi public-launch implementation (2026-09-19)

This entry supersedes the historical scope summaries below for current work. The launch is **not enabled**. Implementation is in `E:\AccountingAppWorktrees\saudi-launch` on `codex/saudi-public-launch`, based on independently verified GitHub main `90e0eaa4896a55c1c8cda1c4101f4ab1323a4a61`.

- Recovered exactly 41 missing tracked files in the original checkout from its own `9f6996d8e8a2468f282675bc54b2659e6523437d` commit. Preserved local instructions/review artifacts and unrelated files. Recovery copies are in the original checkout's `.codex-logs/source-recovery-20260919`.
- Reused current-main billing, session, outbox, recovery/storage, and ZATCA foundations. Installed fresh locked dependencies on verified Node 22.23.2; `.nvmrc` pins the local toolchain. Builds and checks run serially under a Windows process-tree ceiling of 18 GiB and 50% CPU; Jest uses one worker and the web build two.
- Added perpetual quantity/value balances and immutable movement snapshots, finalized discounted base-cost allocations, source-cost returns, cost-preserving transfers, negative/backdated/closed-period rejection, durable receipt/issue/transfer retry keys, and explicit reviewed movement accounting. Fiscal close/lock requires inventory reconciliation. Legacy stock fails closed pending a reviewed cutover; new production organizations start clean.
- Added gated signup/email verification/organization/trial resumption, public catalog, owner trial activation, SAR 149/3-seat and SAR 299/10-seat monthly plans, 14-day no-card trials, request-time expiry/read-only enforcement, and Stripe test-mode Checkout/portal/webhook/reconciliation plumbing. Live keys remain rejected. Local contract tests are not a Stripe network/payment proof.
- Added separate API/web/worker deployment sources, same-origin routing with the Next locale exception, private-storage and migration-role setup instructions, a hosted restore comparison tool, and launch evidence preflight. Durable ZATCA retry ownership now survives process restarts; official simulation and production onboarding are still separate external work.
- The owner selected a UAE seller and directed identity discovery to [ledgerbyte.io](https://ledgerbyte.io/contact). The September 19 website check confirms the published contact `info@ledgerbyte.io` and Sharjah address; it does not verify the exact registered seller, tax identity or Stripe merchant account. `app.ledgerbyte.io` remains a proposed hostname; the marketing domain is unchanged. DNS control, sender authentication and support coverage are unverified. Stripe connector discovery required reauthentication; no DigitalOcean project or test credentials were found in the process environment.

The final local results are recorded in [Saudi launch evidence](docs/production/SAUDI_LAUNCH_IMPLEMENTATION.md): API regression 3,112 tests, web regression 904 tests, fresh-database proof 299 tests after all 115 migrations, final inventory regression 92 tests plus two zero-cost database scenarios, and English/Arabic browser acceptance. API/web builds, shared package tests, compatibility/security guards and the production dependency audit passed; the audit reports zero critical/high findings. The original full-gate process stopped at old web failures, subsequently repaired and rerun with the remaining components serially. Ordinary API runs skip 67 opt-in database tests; separate disposable database proofs cover the named launch scenarios. Three external SDK/JDK/certificate oracle tests remain skipped, and container execution was unavailable because Docker's daemon was stopped. All local fixtures are synthetic; these results do not establish accountant, hosted-provider or official ZATCA acceptance.

Remaining release gates: verified seller/merchant match and tax/terms/privacy identity; named Saudi accountant findings; official ZATCA Simulation followed by production custody, durable issuance integration and applicable production validation; Stripe test payment/failure/recovery followed by a reviewed live implementation; authenticated transactional email and bounce/alert handling; reviewed hosting region and final budget; container startup; hosted tenant isolation, object/database restore and rollback; and bounded capacity rehearsal. No hosted deployment, real email, live collection, accountant contact, or official ZATCA request has occurred in this implementation. See [implementation and owner action packet](docs/production/SAUDI_LAUNCH_IMPLEMENTATION.md), [inventory contract](docs/development/PERPETUAL_INVENTORY_VALUATION.md), and [deployment runbook](infra/deployment/README.md).

Next recommended prompt title: **Complete Saudi launch provider setup and hosted rehearsal**. Start with the documented seller/accountant/ZATCA operator inputs and restricted Stripe test/DigitalOcean access; prove payment, email, tenant isolation, measured recovery and rollback before considering public access. This branch changes local accounting and gated signup/billing behavior, with three additive migrations for review; no existing hosted database or customer data has been changed. Draft PR review and merge remain outstanding.
