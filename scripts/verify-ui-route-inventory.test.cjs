...
test("UI route inventory stays aligned with the shipped page and canonical route counts", () => {
  ...
  assert.match(output, /216 page modules, 98 canonical routes \(94 active, 4 planned\)/);
  assert.match(output, /Fixed-assets governance: 11 page modules \(2 canonical active routes, 9 inventory-only modules\)/);
