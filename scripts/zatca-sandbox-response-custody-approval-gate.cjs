...
const GATE_DOC_PATH = "docs/zatca/SANDBOX_RESPONSE_CUSTODY_APPROVAL_GATE.md";
const ENVIRONMENT = "LOCAL_SANDBOX_RESPONSE_CUSTODY_APPROVAL_GATE_METADATA_ONLY";
...
const DANGEROUS_ALLOWANCES = [
  ...
  {
    label: "DANGEROUS_ALLOWANCE_RESPONSE_BODY_CUSTODY",
    pattern: /response\s+b(?:ody|odies)\s+(?:may|can|should|will|are allowed to)\s+(?:be\s+)?(?:stored|persisted|custodied|retained|archived|forwarded|exported)/i,
  },
  {
  ...
  {
    label: "DANGEROUS_ALLOWANCE_RESPONSE_BODY_HASHING_OR_SUMMARIZATION",
    pattern: /response\s+b(?:ody|odies)\s+(?:may|can|should|will|are allowed to)\s+(?:be\s+)?(?:hashed|redacted|summarized)/i,
  },
  {
    label: "DANGEROUS_ALLOWANCE_RESPONSE_PERSISTENCE",
    pattern: /response\s+(?:storage|persistence)\s+(?:may|can|should|will|is allowed to)\s+(?:be\s+)?(?:enabled|stored|persisted|written)/i,
  },
  {
  ...
  {
    label: "DANGEROUS_ALLOWANCE_DATABASE_WRITE",
...
function usage() {
  return [
    "Usage:",
    "  node scripts/zatca-sandbox-response-custody-approval-gate.cjs --json",
    "  node scripts/zatca-sandbox-response-custody-approval-gate.cjs --json --strict",
    '  node scripts/zatca-sandbox-response-custody-approval-gate.cjs --json --approval-phrase "<text>" --metadata-only',
    "  node scripts/zatca-sandbox-response-custody-approval-gate.cjs --gate-doc <path> --json",
    "",
    "This guard validates committed policy text only.",
    "It never creates request bodies, executes adapters, sends network requests, receives or processes response bodies, stores response custody, executes custody providers, writes to secret managers, writes to databases, writes to object storage, reads env secrets, writes files, enables signing, enables clearance/reporting, enables PDF-A3, or claims production compliance.",
  ].join("\n");
}

function buildSandboxResponseCustodyApprovalGate(options = {}) {
  ...
  let status = "SANDBOX_RESPONSE_CUSTODY_APPROVAL_BLOCKED";
  if (!docsPolicyPassed) {
    status = "SANDBOX_RESPONSE_CUSTODY_APPROVAL_POLICY_BLOCKED";
  } else if (approvalPhraseProvided && !approvalPhraseMatched) {
    status = "SANDBOX_RESPONSE_CUSTODY_APPROVAL_INVALID_PHRASE";
    blockers.push("BLOCKED_INVALID_APPROVAL_PHRASE");
  } else if (approvalPhraseMatched && !metadataOnlyFlagProvided) {
    status = "SANDBOX_RESPONSE_CUSTODY_APPROVAL_METADATA_ONLY_FLAG_REQUIRED";
    blockers.push("BLOCKED_METADATA_ONLY_FLAG_REQUIRED");
  } else if (approvalPhraseMatched && metadataOnlyFlagProvided) {
    status = "SANDBOX_RESPONSE_CUSTODY_APPROVAL_RECOGNIZED_BUT_EXECUTION_BLOCKED";
  ...
  return {
    ...
    responseCustodyAuthorizedNow: false,
    ...
    warnings,
    nextApprovalBoundary: "ZATCA sandbox CSID storage approval gate",
    recommendedNextPrompt: "ZATCA sandbox CSID storage approval gate",
  };
}
...
function renderText(result) {
  return [
    `ZATCA sandbox response custody approval gate: ${result.status}`,
...
function main(argv = process.argv.slice(2), io = { stdout: process.stdout, stderr: process.stderr }) {
  ...
  try {
    ...
    const payload = {
      status: "SANDBOX_RESPONSE_CUSTODY_APPROVAL_INVALID_ARGUMENT",
      environment: ENVIRONMENT,
      metadataOnly: true,
      envSecretsRead: false,
      dbAccessAttempted: false,
  ...
  return args.strict && result.status !== "SANDBOX_RESPONSE_CUSTODY_APPROVAL_RECOGNIZED_BUT_EXECUTION_BLOCKED" ? 1 : args.strict ? 1 : 0;
}
...
module.exports = {
  APPROVAL_PHRASE,
  buildSandboxResponseCustodyApprovalGate,
  parseArgs,
};
