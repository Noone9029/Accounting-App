...
const GATE_DOC_PATH = "docs/zatca/SANDBOX_NETWORK_REQUEST_APPROVAL_GATE.md";
const ENVIRONMENT = "LOCAL_SANDBOX_NETWORK_REQUEST_APPROVAL_GATE_METADATA_ONLY";
...
function usage() {
  return [
    "Usage:",
    "  node scripts/zatca-sandbox-network-request-approval-gate.cjs --json",
    "  node scripts/zatca-sandbox-network-request-approval-gate.cjs --json --strict",
    '  node scripts/zatca-sandbox-network-request-approval-gate.cjs --json --approval-phrase "<text>" --metadata-only',
    "  node scripts/zatca-sandbox-network-request-approval-gate.cjs --gate-doc <path> --json",
    "",
    "This guard validates committed policy text only.",
    "It never creates request bodies, executes adapters, sends network requests, reads env secrets, opens network connections, writes files, processes responses, signs XML, enables clearance/reporting, enables PDF-A3, or claims production compliance.",
  ].join("\n");
}

function buildSandboxNetworkRequestApprovalGate(options = {}) {
  ...
  let status = "SANDBOX_NETWORK_REQUEST_APPROVAL_BLOCKED";
  if (!docsPolicyPassed) {
    status = "SANDBOX_NETWORK_REQUEST_APPROVAL_POLICY_BLOCKED";
  } else if (approvalPhraseProvided && !approvalPhraseMatched) {
    status = "SANDBOX_NETWORK_REQUEST_APPROVAL_INVALID_PHRASE";
    blockers.push("BLOCKED_INVALID_APPROVAL_PHRASE");
  } else if (approvalPhraseMatched && !metadataOnlyFlagProvided) {
    status = "SANDBOX_NETWORK_REQUEST_APPROVAL_METADATA_ONLY_FLAG_REQUIRED";
    blockers.push("BLOCKED_METADATA_ONLY_FLAG_REQUIRED");
  } else if (approvalPhraseMatched && metadataOnlyFlagProvided) {
    status = "SANDBOX_NETWORK_REQUEST_APPROVAL_RECOGNIZED_BUT_EXECUTION_BLOCKED";
  ...
  return {
    ...
    sandboxNetworkRequestAuthorizedNow: false,
    ...
    warnings,
    nextApprovalBoundary: "ZATCA sandbox response processing approval gate",
    recommendedNextPrompt: "ZATCA sandbox response processing approval gate",
  };
}
...
function renderText(result) {
  return [
    `ZATCA sandbox network request approval gate: ${result.status}`,
...
function main(argv = process.argv.slice(2), io = { stdout: process.stdout, stderr: process.stderr }) {
  ...
  try {
    ...
    const payload = {
      status: "SANDBOX_NETWORK_REQUEST_APPROVAL_INVALID_ARGUMENT",
      environment: ENVIRONMENT,
      metadataOnly: true,
      envSecretsRead: false,
      dbAccessAttempted: false,
  ...
  const result = buildSandboxNetworkRequestApprovalGate({
    cwd: process.cwd(),
  ...
  return args.strict && result.status !== "SANDBOX_NETWORK_REQUEST_APPROVAL_RECOGNIZED_BUT_EXECUTION_BLOCKED" ? 1 : args.strict ? 1 : 0;
}
...
module.exports = {
  APPROVAL_PHRASE,
  buildSandboxNetworkRequestApprovalGate,
  parseArgs,
};
