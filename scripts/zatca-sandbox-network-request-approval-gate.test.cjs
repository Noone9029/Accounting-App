...
const SCRIPT_PATH = path.join(__dirname, "zatca-sandbox-network-request-approval-gate.cjs");
...
test("blocked by default", () => {
  ...
  assert.equal(payload.status, "SANDBOX_NETWORK_REQUEST_APPROVAL_BLOCKED");
...
test("recognizes exact phrase only as metadata approval and keeps execution blocked", () => {
  ...
  assert.equal(result.status, 1);
  const payload = JSON.parse(result.stdout);
  assert.equal(payload.status, "SANDBOX_NETWORK_REQUEST_APPROVAL_RECOGNIZED_BUT_EXECUTION_BLOCKED");
...
test("exact phrase without metadata-only flag is rejected", () => {
  const { buildSandboxNetworkRequestApprovalGate } = loadScriptWithNetworkTrap();
  ...
  assert.equal(result.status, "SANDBOX_NETWORK_REQUEST_APPROVAL_METADATA_ONLY_FLAG_REQUIRED");
...
test("guard rejects dangerous sample text that allows network execution, adapter execution, request body creation, OTP inclusion, CSID request, signing, response processing, or production compliance", () => {
  const { buildSandboxNetworkRequestApprovalGate } = loadScriptWithNetworkTrap();
  ...
  const result = buildSandboxNetworkRequestApprovalGate({ cwd: repo });

  assert.equal(result.status, "SANDBOX_NETWORK_REQUEST_APPROVAL_POLICY_BLOCKED");
...
test("no network, DB, env secret, or command execution behavior exists", () => {
  const { buildSandboxNetworkRequestApprovalGate } = loadScriptWithNetworkTrap();
  const result = buildSandboxNetworkRequestApprovalGate({ cwd: createRepo(completeGateDoc()) });
...
test("package script command works", () => {
  const command =
    process.platform === "win32"
      ? {
          file: "cmd.exe",
          args: ["/d", "/s", "/c", "corepack pnpm zatca:sandbox-network-request-approval-gate -- --json --strict"],
        }
      : {
          file: "corepack",
          args: ["pnpm", "zatca:sandbox-network-request-approval-gate", "--", "--json", "--strict"],
        };
...
function loadScriptWithNetworkTrap() {
  ...
  Module._load = function guardedLoad(request, parent, isMain) {
    if (networkModules.has(request)) {
      throw new Error(`Network module should not be loaded by sandbox network request approval gate: ${request}`);
    }
...
function createRepo(gateDocText) {
  const repo = fs.mkdtempSync(path.join(os.tmpdir(), "zatca-network-request-gate-"));
  fs.writeFileSync(path.join(repo, "package.json"), "{}\n");
  writeText(repo, "docs/zatca/SANDBOX_NETWORK_REQUEST_APPROVAL_GATE.md", gateDocText);
  return repo;
}
